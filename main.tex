...
\noindent
Table \ref{tab:edge_types} below shows the different edge types for dual hives 
and honeycombs, dual to each other. 
...
as the honeycomb is non-degenerated and for otherwise condition $(2)$ of 
...
\begin{proof}
    ...
    $\honey_{n,d}^{G^C}(\alpha,\beta,\gamma)\cap \honey_{n,d}^{G^{C'}}(\alpha,\beta,\gamma)=\emptyset$ for $C\not=C'$. 
    Therefore,
    $$\honey_{n,d}=\bigsqcup_{C\text{ color map}}\honey_{n,d}^{G^C} \ .$$
    \noindent
    ...
    Hence, the matrix of the map $(\mathcal{L}\circ \rho_{C})^{-1}$ has integer coordinates in the canonical bases.
\end{proof}
...
\begin{proof}
    ...
    Hence, 
    $$\mathcal{L}\left(\honey_{n,d}^{G^C}(\alpha,\beta,\gamma)\right) = 
    \Psi^{-1} \left(\mathcal{H}^C(\alpha,\beta,\gamma) \cap \mathcal{K}_{\leq} \right)
...
with the ones of the previous section to yields the following formula. 
