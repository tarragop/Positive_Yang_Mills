...
\section{Triangular honeycombs as decorated graphs and a canonical measure \\(provisional title)}


We would like to see a triangular honeycomb
as a honeycomb graph endowed
with a flow (an antisymmetric function on its
edge set). 
Let us consider a triangular honeycomb 
with underlying honeycomb graph $G=(V,E,c)$
and let us denote $\vec E = \{(a,b) \in V\times V \colon
\{a,b\} \in E\}$
its oriented edge set.  The triangular honeycomb
information of $G$, i.e., the precise
drawing on the equilateral triangle of size $1$, 
will be obtained
once we fix the distance of each
edge to the side of the triangle parallel to it 
(\textcolor{blue}{the edge type is data or is it somehow determined by some other data?}).
This is a multiple of the height map
$L:E \to [0,1]$ given in
Definition \ref{def:triangular_honey}.
Recalling (\textcolor{blue}{TO FIX}) that we have
chosen the function
$\beta: V_{>1} \to \{-1,1\}$
that encodes the vertex type,
we define a map
$\lambda: \vec E \to \mathbb R$ by
\[\lambda(a,b) = 
	\left\{\begin{array}{ll} 
 L(\{a,b\}) &\mbox{ if } \beta(a)=1 				\\
	 - L(\{a,b\}) & \mbox{ if } \beta(a) =-1 		
\end{array}
\right.\]
Then, $\lambda$ is antisymmetric and satisfies 
that, for every $v \in V_{>1}$,
\begin{equation}
	\label{eq:DivOfLambda}
	\sum_{x \sim v} \lambda (v,x) = \beta(v).
\end{equation}
...
to define a measure on the space of triangular honeycombs.
...
real functions $\omega$ on $\vec E$, i.e.,
the ones
satisfying $\omega(a,b) = -\omega(b,a)$
...
rewrites as
	$(\pi \circ \mathrm{div}) \lambda = \beta$
	with $\pi: \Omega^0(V) \to \Omega^0(V_{>1})$
	the restriction map to $V_{>1}$.
	 We will be interested in two kind of solution space. The first one would be the plain
	 \[\mathcal F(\beta)
	 = 	\{\lambda \in \Omega^1(G):
	 (\pi \circ \mathrm{div}) \lambda = \beta
	 \}.\]
	 We can quickly see that
	$\mathcal F(\beta)$ is always non-empty
	by noticing that
	$\mathrm{Im}(\pi \circ \mathrm{div})
	= \mathrm{Ker}(\mathrm{div}^* \circ \pi^*)^\perp$
	where $\pi^*$, the adjoint of $\pi$, is
	the extension by zero operator
	$i: \Omega^0(V_{>1}) \to \Omega^0(V)$
	and, by definition, $\mathrm{div}^* = -d$.
	A function $f \in \mathrm{Ker}(d \circ i)$
	would satisfy that $i(f)$ is constant since
	$G$ is connected, but 
	since $i(f)|_{V_1} = 0$
	and $V_1$ is non-empty, $f$ has to be zero.
	So, $\mathrm{Im}(\pi\circ \mathrm{div}) = 0^\perp 
	= \Omega^0(G)$.
	We may calculate the dimension 
	of $\mathcal F(\beta)$ by using that $\mathcal F(0) = \mathrm{Ker}(\pi \circ \mathrm{div}) = 
	(\mathrm{Im}(\mathrm{div}^* \circ \pi^*))^\perp =
	(\mathrm{Im}(d \circ i))^\perp$.
	Then, by using that the dimension of 
	$\mathrm{Im}(d\circ i)$ is
	 $|V_{>1}|$ because of the injectivity, we can write
		\[\mathrm{dim}\big(\mathcal F(\beta)
		\big)
		= |E| - \mathrm{dim}(\mathrm{Im}(d \circ i)) 
		=
		|E| - |V_{>1}|. \]
	For the second kind of solution space one we add boundary conditions.
	 Let us take 
	 $\gamma: V_1 \to \mathbb R$ and
	 for every vertex $x \in V_1$ let us denote by
	 $\vec x$ the unique edge ending at $x$. We define
	 \[\mathcal F(\beta; \gamma)
	= 	\{\lambda \in \mathcal F(\beta):
	\lambda(\vec x) = \gamma(x) \mbox{ for every }
	x \in V_1
	\}.\]
This solution space may be empty since we
always have
	$\sum_{v \in V_{>1}} \mathrm{div} \lambda (v)
	= \sum_{x \in V_1} \lambda(\vec x)$
	...
	$\lambda_0 \in \mathcal F(\beta;
	\gamma)$ we have
	$\mathcal F(\beta;\gamma) = 
	\lambda_0 + \mathcal F(0;0)$ and
	$\mathcal F(0;0)$ is precisely
	$\mathrm{Ker}(\mathrm{div})$
	since the boundary conditions can be
	written as $\mathrm{div}\lambda (x)= 0$
	for $x \in V_1$. So, if
	$\mathcal F(\beta;\gamma)$ is non-empty,
	we can calculate
	\[\mathrm{dim}(\mathcal F(\beta;\gamma))
...
\begin{proposition} \label{prop:BijectionTrees}
	Take $\gamma: V_1 \to \mathbb R$. The set
	$\mathcal F(\beta;\gamma) $ is not empty if and only if
\begin{equation} \label{eq:ExistenceCondition}
\sum_{x \in V_1} \gamma(x) 
	= \sum_{v \in V>1} \beta(v).
\end{equation}
	Moreover, if $S \subset E$
	and  $\Omega^1(S)$ denotes the space of antisymmetric
	functions on $\vec S \subset \vec E$, the composition
	\[\varphi_S:\mathcal F(\beta;\gamma) \xrightarrow[]{
	\mbox{\tiny inclusion}}
	\Omega^1(G) 
	\xrightarrow[]{
		\mbox{\tiny restriction}} \Omega^1(S)\]
	is a bijection if and only if $\mathcal F(\beta;\gamma)
	\neq \emptyset$
	 and
	$(V,E \setminus S)$ is a spanning tree.
	In case $\varphi_S$ is a bijection,
	\[(\varphi_S)_* \mathcal Leb_{\mathcal F(\beta;\gamma)}	= \big(\hspace{-0.5mm} \sqrt{\# \mbox{ Spanning trees of } G}	\big)\,
...
the condition
	...
	is most easily determined by the boundary conditions.
	Those
	are the ones with an endpoint whose
	...
	ending at $x$, a solution should satisfy
	\[\bigg(\sum_{y \in V_{1,x}}\gamma(y)
	\bigg) - \lambda(e) 
	= \beta(x)\]
	which determines $\lambda$
	at $e$. Now, we consider the graph 
	$G\setminus V_{1,x} $, define
	$\gamma(x) =-\beta(x) +
	\sum_{y \in V_{1,x}}\gamma(y) 
	$
	and notice that
	\eqref{eq:ExistenceCondition}
	...
	is precisely the
	equation 	 
	$(\pi \circ \mathrm{div}) \lambda = \beta$. By this procedure
	we have seen that, for a tree, 
	in case there is a solution, it is unique.
...
%	if $G$ is not connected) 
	...
	$\lambda \in \mathcal F(\beta;\gamma)$
	satisfying $\lambda|_{\vec S} = \widetilde \lambda$.
	To 
	be able to forget the edges in $S$ we change $\beta$ to
	$\beta_{new}:V_{>1} \to \mathbb R$ given by
		\[\beta_{\tiny new}(x) =\beta(x) -
	\sum_{e \in \vec S, \underline{e} = x}
	\widetilde \lambda(e)\]
	so that if the divergence of $\lambda$
	in $T$ at $x$ is
	$\beta_{new}(x)$, the divergence of the extension
	by $\widetilde \lambda$
	of $\lambda$
	in $G$ at $x$ would be 
	$\beta(x)$.
	Notice that the sum of $\beta_{new}(x)$
	for $x \in V_{>1}$ is the same as the sum of 
	$\beta(x)$.
	This holds because each edge in $S$ is not connected to a vertex in $V_1$
	so that
	it appears twice in the sum,
	once with each orientation.
	Since $T$ is spanning, each vertex in $V_1$ 
	is a leaf of $T$ so that we define
	$\gamma_{new}(x) = \gamma(x)$
	for $x \in V_1$.
	Nevertheless, there may be leaves of $T$ that are not in 
	$V_1$ which forces us
	to add boundary conditions.
	We define $\gamma_{new}(x) = -\beta_{new}(x)$
	for $x$ a leaf of $T$ which is not a leaf of $G$.
	Then 
	\eqref{eq:ExistenceCondition} holds
	for $T$ and $(\beta_{new};\gamma_{new})$, and we may
	find a unique solution $\lambda \in \mathcal F(\beta_{new},\gamma_{new})$ on $T$.	
	We extend $\lambda$ by $\widetilde 
	\lambda$ and remark that it is a solution in
	$\mathcal F(\beta,\gamma)$ since
	at the vertices that are not leaves of $T$
	the fact of being a solution remains valid
	because of the definition of
	$\beta_{new}$ and at
	 a leaf $x$ of $T$ that is not
	a leaf of $G$ it is also a solution
	due to the boundary condition. On the other hand,
	if $\lambda$ is a solution in $\mathcal F(\beta;\gamma)$
	its restriction to the directed edge set of $T$
	is a solution in $\mathcal F(\beta_{new};\gamma_{new})$
	so that there is only 
	one solution $\lambda$ in $\mathcal F(\beta;\gamma)$
	satisfying $\lambda|_{\vec S} = \widetilde \lambda$.
	Notice that this already
	shows that $\varphi_S$ is a bijection
	when $(V,E\setminus S)$ is a spanning tree of $G$.
	Another intuitive approach that replaces
	the need to change $\beta$ and $\gamma$ would be 
	to break each edge in $S$ into two edges to obtain
	two new leaves, then $\widetilde \lambda$ would
	become
	boundary conditions
	for these new leaves and we may construct,
	in that way, a new tree containing $T$ where
	a solution  would be
	exactly the same as a solution on $G$.
	\\
	\\
	It is clear that if $\varphi_S$ is a bijection
	then $\mathcal F(\beta;\gamma)$ is not empty
	because $\Omega^1(S)$ would be empty which
	is impossible. So, for
	the rest of the proof we may assume that
	$\mathcal F(\beta;\gamma)$ is not empty.
	By taking any $\lambda_0 \in \mathcal F(\beta;\gamma)$, using that
	$\mathcal F(\beta;\gamma)
	= \mathcal F(0;0) + \lambda_0$ 
	and that
	the map
	$\mathcal F(\beta;\gamma) \to \Omega^1(S)$
	is a composition
	\[\mathcal F(\beta;\gamma)
	...
	Such $\lambda$ would belong to $\mathcal F(0;0)$
	...
	We could have also used a dimension argument
	to see that $S$ cannot be too big.
...
%	is connected.
	...
	\[(\varphi_S)_* \mathcal Leb_{\mathcal F(0;0)}	= C	\mathcal Leb_{\Omega^1(S)}\]
	Denoting the dimension of
	$\mathcal F(0;0)$ by $k = |E|-|V| +1$,
	...
	$\{d \hat v \}_{v \in V \setminus \{v_0\}}$,
	where we recall that
	$\hat v$ is the function which is $1$ at $v$
	and $0$ elsewhere.
	Since $\sum_{v \in V \setminus \{v_0\}}
	d\hat v + d\hat v_0 = d 1= 0$, this family
	generates $\mathrm{Im}(d)$. Now,
	we obtain an element of
	$\Lambda^k \mathcal F(0;0)$ by taking 
	$*(\wedge_{v \in V\setminus \{v_0\}} d \hat v)$,
	...
	using that 
	$ \wedge_{e \in S} \hat e = \pm 
	* (\wedge_{e \in E\setminus S} \hat e)  $,
	\[
	\big<  \hspace{-0.5mm} *(\wedge_{v \in V\setminus \{v_0\}} d \hat v),
	 \wedge_{e \in S} \hat e \, \big>
	 = \pm 
	 \big< \hspace{-0.5mm}  \wedge_{v \in V\setminus \{v_0\}} d \hat v,
	 \wedge_{e \in E\setminus S} \hat e \, \big>
	 = \pm\mathrm{det}\langle d \hat v, \hat e \rangle
	...
	\langle d \widehat {\sigma(e)}, \hat e \rangle$,
	where the sign $(-1)^\sigma$ is only defined
	up to an overall sign. Notice that
	$\langle d \hat x, \hat e \rangle = \pm 1$ if and only
	if $x$ is an endpoint of $e$ and, if not,
	$\langle d \hat x, \hat e \rangle = 0$.
	...
	$*(\wedge_{v \in V\setminus \{v_0\}} d \hat v)$ is not
	zero so the explicit formula for $w_1$
	as the normalized $*(\wedge_{v \in V\setminus \{v_0\}} d \hat v)$ works.
...
\begin{proof}[Proof of Proposition
	...
	V_{>1}} d \hat v),
	 \wedge_{e \in S} \hat e \, \big>
	 = \pm 
	 \big< \hspace{-0.5mm}  \wedge_{v \in V_{>1}} d \hat v,
	 \wedge_{e \in E\setminus S} \hat e \, \big>
	 = \pm\mathrm{det}\langle d \hat v, \hat e \rangle
	 _{v \in V_{>1}, e \in E\setminus S}.
	 \]
	 In the calculation
	 of the determinant
	 we sum over
	 bijections 
	 $\sigma:E\setminus S \to V_{>1}$
	 and the only contributions
	$ \langle d \hat v, \hat e \rangle$
	arise when 
...
where $V_{>1}$ denotes the set of vertices having degree at least two in $G$.
