...
of the corresponding punctured oriented surface, while the collection of geodesics is a colored generalization of the honeycomb model introduced by 
...
Roughly speaking, this formula has been obtained by following the initial method of Verlinde together with the recent results on the computation 
...
boundary components $L_1,\ldots,L_p$, for which the holonomy around 
each $L_i$, $1\leqslant i\leqslant p$, belongs to 
...
$\mathcal{H}_{reg}$.
\\
We associate to $(S,\Gamma_1,\ldots,\Gamma_p)$ a labeled finite directed graph $\mathcal{T}=(V,E)$ such that vertices are labeled by $\mathbb{R}_{\geqslant 0} \times \mathbb{N}$ and directed edges are labeled by $\mathcal{H}_{reg}$ as follows :
\begin{itemize}
\item the set $V$ of vertices of $\mathcal{T}$ is the set of connected components of $S\setminus \bigcup_{i=1}^p\Gamma_i$. 
Each vertex $v\in V$ is labeled $(A_v,g_v)$ where $A_v$ is the area of the corresponding connected component and $g_v$ is its genus.
\item For $v_1,v_2\in V$, there is a direct edge $e$ between $v_1$ and $v_2$ 
if $v_1$ and $v_2$ are boundary components of a loop. Since loops of $\mathcal{L}$ are non-intersecting, each boundary component corresponds to a unique loop $\Gamma_j$ of $\mathcal{L}$, and we label the corresponding edge $e$ by $\alpha_j$ of $1-\alpha_j$ depending on the orientation of the curve around the surface $v_1$.
\end{itemize}

\noindent
We refer to Figure \ref{fig:fig_loops} for an example of such a configuration of loops and its corresponding 
edge-labeled tree.In the following, let us denote by $d_v$ the degree of a vertex $v\in V$. For $T \geqslant 0$ and $x,y \in \mathcal
H$, let us denote by $p_T(x,y)$ the projection of the heat kernel 
on $\U(n)$ to $\mathcal{H}$.
...
\begin{figure}[h!]
    ...
    \begin{minipage}{0.35\textwidth}
        \centering
        \includegraphics[width=\linewidth]{images/tree_surface_loops.pdf}
    \end{minipage}
    \caption{A disjoint loops configurations and its corresponding edge-labeled tree}
    \label{fig:fig_loops}
\end{figure}
...
\begin{lemma}[Flow of honeycomb]
    ...
    \cap (K_{G_1}\times \dots\times K_{G_N}) \ ,$$
    where $i(G)=(G_1,\ldots,G_N)$ and where $\Omega^1(G)$ 
    is identified with $\prod_{i=1}^N\Omega^1(G_i)$. 
\end{lemma}
...
\noindent
We then write $K_{G}=K_{G_1}\times \dots\times K_{G_N}$.
...
The goal of this section is to derive an explicit volume formula 
...
\begin{definition}[Disjoint loops configuration]
A \textit{disjoint loops configuration} $\mathcal{L}=(S,\Gamma_1,\ldots,\Gamma_p)$ 
is the data of a compact oriented surface together with $p$ disjoints 
Jordan curves $\Gamma_1,\ldots,\Gamma_p$ on $S$ and for each 
$\Gamma_i, \,1\leqslant i\leqslant p$ an element $\alpha_i\in \mathcal{H}_{reg}$.
A \textit{skeleton} is the data of a labeled finite graph $\mathcal{T}=(V,E)$ such vertices are labeled by $\R_{\geqslant 0} \times \mathbb{N} $ 
and edges are labeled by elements of $\mathcal{H}_{reg}$.
\end{definition} 

To each disjoint loops configuration $\mathcal{L}$, one associates a skeleton $\mathcal{T}(\mathcal{L})$ as follows :
\begin{itemize}
\item the set $V$ of vertices of $\mathcal{T}(\mathcal{L})$ is the set of connected components of $S\setminus \bigcup_{i=1}^p\Gamma_i$. 
Each vertex $v\in V$ is labeled $(A_v,g_v)$ where $A_v \geqslant 0$ is the area of the corresponding connected component and $g_v \in \N$ is its genus.
\item For $v_1,v_2\in V$, there is an edge $e$ between $v_1$ and $v_2$ 
if $v_1$ and $v_2$ are boundary components of a loop.
Since loops of $\mathcal{L}$ are non-intersecting, each boundary component corresponds to a unique loop $\Gamma_j$ of $\mathcal{L}$, and then we label $\alpha_j$ the edge $e$.
\end{itemize}

\noindent
In this section, 
for $T \geqslant 0$ and $x,y \in \SU(n)$, let us denote by $p_T(x,y)$ the projection of the heat kernel 
on $\U(n)$ to $\mathcal{H}$ and $p_T(x) = p_T(\mathrm{id}, x)$.
...
\begin{proposition}[Volume formula for Yang-Mills partition function]
    ...
    \begin{equation*}
        ...
        = \int_\mathcal{H} Z_{g, p, 0}(\alpha_1, \dots, \alpha_{p-1}, u) \, 
...
$$Z_{g, p, T}(\alpha_1, \dots, \alpha_p)=\int_{\mathcal{H}} Z_{g, p, 0}(u_1, \dots, u_p) \, 
        \prod_{i=1}^pZ_{0, 2, \frac{T}{p}}(u_i^{-1}, \alpha_i) \prod_{\ell=1}^p p_{t_\ell}(\alpha_\ell) \prod_{i=1}^p \left(\Delta(u_i)^2\diff u_i \right)\ ,$$
where $Z_{g,p,0}(u_1, \dots, u_p)$, 
...
The corollary is then a deduction of the latter proposition, the Markov properties of the Yang-Mills marginal, see \cite[Proposition 5.1.2]{levy2003yang} and Theorem \ref{th:Z_g_p_0}. The proof is done by induction on the number $M$ of vertices of the skeleton $\mathcal{T}(\mathcal{L})$. If $M=1$, any curve is 
\end{proof}
