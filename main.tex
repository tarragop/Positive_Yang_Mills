...
edge to the side of the triangle parallel to it (the edge type is data or is it somehow determined by some other data?).
This is a multiple of the height map
$L:E \to [0,1]$ given in
Definition \ref{def:triangular_honey}.
Recalling (TO FIX) that we have
chosen the function
$\beta: V_{>1} \to \{-1,1\}$
...
%	if $G$ is not connected) 
	...
	to see that $S$ cannot be too small.
...
%	is connected.
	...
	\[(\varphi_S)_* \mathcal Leb_{\mathcal F(\beta;\gamma)}	= C	\mathcal Leb_{\Omega^1(S)}\]
...
\begin{proof}[Proof of Proposition
	...
	contained a path
	$(v_1,\dots,v_k)$ with distinct vertices
	and $v_1,v_k \in V_1$,
	we could define
	...
	$(V,E\setminus S)$ contains at most one vertex
	in $V_1$. 
