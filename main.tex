...
rewrites as
	...
	being the restriction to $V_{>1}$.
	Since 
	$\pi$ is adjoint to the extension by zero operator
	$i:\Omega^0(V_{>1}) \to \Omega^0(V)$,
	we know that
	$\pi \circ \mathrm{div} = (d \circ i)^*$
	so that
	\[\mathrm{dim}\big(\mathrm{Ker}(\pi \circ \mathrm{div})
	\big)
	=\mathrm{codim}\big(\mathrm{Image}(d \circ i)
	\big) 
	= 
	|E| - \mathrm{dim}\big(\mathrm{Image}(d \circ i)
	\big). \]
	Since $d\circ i$ is injective
	because every vertex of $G$ is connected
	to a vertex in $V_1$, we can write
	\[\mathrm{dim}\big(\mathrm{Ker}(\pi \circ \mathrm{div})
	\big)
	= 
	|E| - |V_{>1}|. \]
	...
	(\pi \circ \mathrm{div}) \lambda = \beta
	 ...
	 (\pi \circ \mathrm{div}) \lambda = \beta
	 \}\]
	 and for the second one we add boundary conditions.
	...
	\}.\]
	To see that 
	$\mathcal F(\beta; \gamma)$ can be empty, we
	remark that
	$\sum_{v \in V_{>1}} \mathrm{div} \lambda (v)
	= \sum_{x \in V_1} \lambda(\vec x)$
	which is reminiscent of
	Stokes' theorem. But, in fact, this is the only
	restriction as seen in the next proposition.
...
the condition
	...
	if $G$ is not connected) and try to solve
	the equation for $T$. 
	We may choose
	...
	could not be surjective. A dimension argument
	...
	$0$ at all other edges.
	
	
	
	...
	Fix a leaf $v_0$
	of $(V,E\setminus S)$ and consider
	the family
