...
rewrites as
	...
	the restriction map to $V_{>1}$.
	 ...
	 as explained in Proposition
	  \ref{prop:BijectionForests} below.
	We may calculate its dimension indirectly by using that $\mathcal F(0) = \mathrm{Ker}(\pi \circ \mathrm{div}) = 
	...
	the extension by zero operator
		...
		and, by definition,
		$\mathrm{div}^* = -d$.
		Notice that $-d\circ i$ is injective
		because 
		the kernel of $d$ consists of constant functions. Then, we can write
...
\end{equation}
	...
	\[(\varphi_S)_* \mathcal Leb_{\mathcal F(\beta;\gamma)}	= \frac{1}{\sqrt{\# \mbox{ Spanning trees of } G}}\,
...
\begin{proposition} \label{prop:BijectionForests}
	...
	\[(\psi_S)_* \mathcal Leb_{\mathcal F(\beta)}	= \frac{1}{\sqrt{\# \mbox{ Forests of } G}}\,
	\mathcal Leb_{\Omega^1(S)}.\]
\end{proposition}
...
%	is connected.
	...
	* (\wedge_{e \in E\setminus S} \hat e)  $,
	...
	ToDo: Find the mistake in the proof or in the statement.
