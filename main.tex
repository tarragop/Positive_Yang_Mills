...
rewrites as
	...
	restriction by the next proposition.
	
\begin{proposition} \label{prop:BijectionTrees}
	Take $\gamma: V_1 \to \mathbb R$. The set
	$\mathcal F(\beta;\lambda) $ is not empty if and only if
...
\end{equation}
	...
	\[\varphi_S:\mathcal F(\beta;\lambda) \xrightarrow[]{
	\mbox{\tiny inclusion}}
	\Omega^1(G) 
	\xrightarrow[]{
	...
	Finally, in case $\varphi_S$ is a bijection,
	\[(\varphi_S)_* \mathcal Leb_{\mathcal F(\beta;\lambda)}	= \frac{1}{\sqrt{\# \mbox{ Spanning trees of } G}}\,
...
the condition
	...
	= \sum_{v \in V>1} \beta(v)$
	is necessary.
	Let us show it is enough.
	Consider first the case where
	$G$ is a tree. To solve
	$ (\pi \circ \mathrm{div}) \lambda = \beta$
	we look for
	the edges where $\lambda$
	is most easily determined. Those
	are the ones adjacent to the leaves of
	$G \setminus V_1$. Take
	a leaf $x \in V_{>1}$ of $G \setminus V_1$.
	Since it is not a leaf of $G$, the set $V_{1,x}$
	of vertices in $V_1$ connected to
	$x$ is non-empty. So, if $e$ denotes
	the edge of $G \setminus V_1$
	ending at $x$, we must have
	...
	$\sum_{x \in V_1} \gamma(x) 
	= \sum_{v \in V>1} \beta(v)$	
	is still satisfied for $V_1$
	the leaves of $G\setminus V_{1,x}$
	and $V_{>1}$ the complement of $V_1$.
	...
	For a general graph $G$ we
	may take a spanning tree $T$ of $G$
	(spanning tree of each of its components
	if $G$ is not connected) and try to solve
	the equation for $T$. 
	To achieve this
	we first notice that each leaf of $G$
	is a leaf of $T$
	since $T$ is spanning.
	Then if $x$ is a leaf of $T$ 
	that is not a leaf of $G$ we
	may define $\gamma(x) = -\beta(x)$.
	Then 
	\eqref{eq:ExistenceCondition} holds
	for $T$ and we may
	find a solution $\lambda$
	for $ (\pi \circ \mathrm{div}) \lambda 
	= 	\beta$ on $T$ with boundary conditions
	given by $\gamma$.	
	Extending $\lambda$
	to zero at the edges that are not in 
	$T$ we get a solution on $G$
	does not affect
	the fact that $\lambda$ is a solution
	at the vertices that are not leaves of $T$.
	Moreover,
	at a leaf $x$ of $T$ that is not
	a leaf of $G$ it is also a solution
	due to the boundary condition
	$\gamma(x) = -\beta(x)$.
	
	From this procedure we can see
	that we may choose
	the values of $\lambda$
	arbitrarily
	at the edges that are not in
	a spanning tree $T$ of $G$. Indeed,
	since we want to find a solution on $T$,
	we may forget
	the edges that are not in $T$
	but change
	$\beta$ at $x$ to
	$\beta_{\tiny new}(x) =\beta(x) -
	 \sum_{e \notin T, \underline{e} = x}
	 \lambda(e)$. Notice that
	 the sum
	of $\beta_{\tiny new}(x)$
	over all vertices $x$
	that are not leaves of $G$
	is the same as the sum of $\beta(x)$.
	This holds because each edge not found
	in $T$ is not connected to a vertex in $V_1$
	so that
	 it appears twice in the sum,
	once with each orientation.
	We find a solution $\lambda$
	on $T$ by adding, as before,
	the boundary condition
	$\gamma(x) = -\beta_{\tiny new}(x)$
	for $x$ a leaf of $T$ that is not a
	leaf of $G$ but we extend it by the 
	values that were fixed outside $T$.
	The next argument also
	implies that we may freely choose
	the values of $\lambda$
	outside of $T$ but
	it also implies the final part
	of the proposition.
