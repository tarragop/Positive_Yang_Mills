...
\begin{abstract}
We provide a manisfestly positive expression for the volume of flat $\U_n$-valued connections on punctured compact Riemann surfaces. This volume is obtained by summing volumes of explicit polytopes describing colored honeycombs on a polygon, in the spirit of the work of Knutson and Tao describing the spectrum of the sum of two hermitian matrices. As a corollary, we also provide a positive formula for marginals of $\U_n$-valued Yang-Mills measures on a compact Riemann surfaces in terms of the probability distribution of an explicit path process.
\end{abstract}
...
Moduli spaces of unitary holomorphic vector bundles have been introduced by Narasimhan and Seshadri \cite{Narasimhan_Seshadri} as a way to classify vector bundles on a compact Riemann surface. As highlighted in the work of Atiyah and Bott \cite{Atiyah_bott_1983}, these  moduli spaces are deeply related to the $\U_n$-valued Yang-Mills measure on the corresponding surface. The latter is a random $\U_n$-valued connection weighted by its curvature, and the critical points then correspond to flat connections, which in turns are in correspondance with the holomorphic vector bundles. As such, there is a natural symplectic volume form on the moduli space of holomorphic bundles (or flat connections) which appears as the a zero-limit temperature of the Yang-Mills functional.

Since the paper of Atiyah and Bott, there have been several works pursuing the goal of a better understanding of either the Yang-Mills measure or the volume form on flat connections. Let us mention in particular two pioneering works that appeared around 1990. The first one, introduced by Verlinde \cite{Verlinde}, expresses the volume form as the limit of the dimension of a vector space of holomorphic sections on the moduli space. This expression is clearly positive as a limit of a positive integers. There is an explicit formula, orginally due to Verlinde and later proved by Bismut and Labourie \cite{bismut5symplectic}, to compute these integers as a sum of complex numbers in the case of the three-holed sphere. 

However, until recently \cite{Buch_Kresch_Purbhoo_Tamvakis}, no manifestly positive formula existed for the dimension of such spaces. The second work, due to Witten \cite{witten1992two}, reduces the case of an arbitrary compact Riemann surface to a three-holed sphere by exploiting a decomposition of pants of the surface. Such surgery procedure has been then rigorously proved several times, see \cite{jeffrey1998intersection} and \cite{meinrenken1999moduli}. Using this method, Witten succesfully obtained a general formula for the volume of flat connections as series of characters of the unitary group. This first result, which is a series of complex numbers, has been improved to an alternating sum of positive numbers by Jeffrey \cite{jeffrey1994extended} and then Meinrenken and Woodward \cite{meinrenken1999moduli}.

In the present paper, we provide a positive formula for the volume of such moduli spaces as  a sum of volumes of explicit finite dimensional polytopes. These polytopes consist of coordinates of a collection of geodesics in a polygonal surface. The surface itself simply reflects the pants decomposition of the corresponding puncture Riemann surface, while the collection of geodesics is a colored generalization of the honeycomb model introduced by Knutson and Tao \cite{knutson2004honeycomb} to describe the spectrum of the sum of two hermitian matrices with prescribed spectra. The fact that our model is a generalization of \cite{knutson2004honeycomb} should not be a surprise, since the solution of the eigenvalue problem for the sum of hermitian matrices yields a particular subexample of a flat connection on a three-holed sphere, see \cite{klyachko1998stable,belkale2008quantum}. Actually, this subexample has been used to get the previously mentioned signed formula for the volume of the moduli spaces on the three-holed sphere, see \cite{meinrenken1999moduli}.


The proof of the present result is mainly based on the positive formula given in \cite{françois2024positiveformulaproductconjugacy} in the case of the three-holed sphere. Roughly speaking, this formula has been obtained by following the initial method of Verlinde together with some recent results on the computation of the structure constants of the quantum cohomology of the Grassmannians \cite{Buch_Kresch_Purbhoo_Tamvakis}. In \cite{françois2024positiveformulaproductconjugacy}, the formula was given in terms of volumes of a so-called coloured hive model, which consists of a union of some explicit but degenerated polytope. 

Note that volumes are always defined up to a constant and when polytopes of a Euclidean space are degenerated, there are several ways to choose this constant from the volume measure of the ambient space : one can either consider the volume induced by the restriction of the Riemann metric or consider the pull-back of the canonical Lebesgue measure by a bijective projection on a subset of a canonical basis. Both definitions agree in the case of a convex body, but they start to differ when degeneracies appear. The choice of a volume form was relatively inconsequent in the case of \cite{knutson2004honeycomb}, since there was only one polytope considered and the change of volume form only amounted to a change by an overall constant. However, we are considering here a sum of volume polytopes, each of them degenerated and being in different subspaces of the ambient space, and we thus have to ensure that the chosen volumes are coherent from one polytope to the other. In \cite{françois2024positiveformulaproductconjugacy}, the correct choice was to use the pull-back corresponding to particular bijective projections. 

It appears that the formulation of \cite{françois2024positiveformulaproductconjugacy} in terms of hive does not fit easily in the surgery formulas of Witten. In order to generalize the case of the three-holed sphere to arbitrary punctured compact Riemann surfaces, our first task has been to reformulate the latter hive model in terms of a colored version of honeycombs. As a byproduct of this reformulation, we also obtain a new volume formula in the case of the three holed sphere which is coordinate free : we provide an explicit relation between the volume form induced by the Riemann metric of the ambient space and any volume induced by a bijective projection on some coordinates. This relation is given by a simple combinatorial number, the number of spanning tree of a graph, in the similar spirit as in \cite{kassel2022determinantal}. Thanks to the new honeycomb model, the surgery formulas of \cite{witten1992two} to compute the volume of flat connections easily translates into a patching of honeycombs, and we thus get the desired positive formula.

 Remark that this formula suggests a simple geometric description of a flat $\U_n$-connection on a compact Riemann surface, up to an overall conjugation by a unitary matrix, by then corresponding honeycomb arrangement. It would be very interesting to provide an explicit measure preserving bijection between the two models. However up to now, such a bijection has been out of reach already for the simplest particular case of the sum of hermitian matrices.

As suggested by \cite{Atiyah_Bott}, volume of flat connections are the building blocks for the computations of the partition function of the two dimensional Yang-Mills measure on compact Riemann surfaces, see \cite[Section 2]{witten1992two}. Using the new positive formulas for flat connections, one can thus easily deduce new formulas for marginals of the Yang-Mills measure which are again manifestly positive. Remark that our formulas are only valid for the holonomy on a set of non-crossing curves, and this gives thus only a partial description of the full Yang-Mills measure. Once again, our result suggests the existence of a measure preserving bijection between a projection of the Yang-Mills measure and a random path processes consisting of Dyson Brownian motions on circle and certain frozen path configurations in polygons, see Remark \ref{rmk:Yang-Mills_volume_conditioned_process}.
...
\subsection{ \texorpdfstring{$(g,p)-$}{(g, p)-}honeycombs}\label{subsec:(g,p)_honey} Let $g,p\geqslant 0$ be integers and let $S$ be a 
...
$\honey^G$ constructed as follows. For any edge $e\in E$, there exists $1\leq i\leq N$ such that 
...
values of $((P_{\vert T_i}(e))_{e\in G_h^i})_{1\leq i\leq N}$
...
For $g,p\geq 0$, Set $\diff \Vol\in\Omega^{n_{g,p}}\left(\mathbb{R}^{E^{(g,p)}}\right)$ 
for the volume form on $n_{g,p}$-dimensional subspaces induced by the 
canonical Riemannian metric on $\mathbb{R}^{E^{(g,p)}}$. Hence, 
\begin{equation}
    \label{eq:volume_R_E}
    \diff \Vol = \sum_{1\leq i_1<\ldots<i_{n_{g,p}} \leqslant n_{e}}\diff x_{i_1} 
...
denotes the absolute value of the Vandermonde determinant of $(e^{ix_j})_{1\leq j\leq n}$.
...
since each set $\mathcal{G}_{d}^{(g,p)}$ is finite and $d\leq n-1$. 
...
There are several other conventions to define the volume of flat connections, depending on the chosen volume on the unitary group (see \cite[Section 4.1]{witten1992two}) and depending on the kind of volume form considered. On the latter convention, remark that we are choosing here the symplectic volume, more adapted to Yang Mills generalizations, rather than the torsion volume introduced in \cite{witten1992two}, see also \cite[Appendix A]{meinrenken1999moduli} for a symplectic point of view on the relation between both volumes. The volume forms are easily related by the explicit factor $\prod_{j=1}^{p} \Delta(\alpha_j)$, see \cite[Section 4.7]{witten1992two}.
...
$L_j,\, 1\leq j\leq p$, and $\frac{(3m-p)n}{2}$ bivalent vertices, 
$n$ of them being on a the boundary of a common triangle while being 
in the interior of the surface. 
\\
\\
Let us introduce some notations. 
First, for $1\leq j\leq p$, let $1 \leqslant s_j \leqslant m $ and 
$ 0 \leqslant \ell_j \leqslant 2$ 
be such that $L_j=\partial_{\ell_j}T^{s_j}$. 
For $1 \leqslant j \leqslant p$, let us denote by $(v^j_m)_{1\leq m\leq n}$ 
...
$$i: \left\lbrace \begin{aligned}  
    ...
    h^{r_j^{(2)}}_{\vert B_j}, 1\leq j\leq N(\mathcal{T}) \right\}\\
...
Suppose the result true for $m\geq 1$, and let $\mathcal{T}$ be a surface 
...
$T^{m+1}=\{-r+se^{2\pi i/3},\,0\leq r,s\leq 1, r+s\leq 1\}$ and that 
$\partial_0T^{m+1}=\{se^{2\pi i/3}\}=L_p$, so that 
$T^{m}=\{re^{\pi i/3}+se^{2\pi i/3},\,0\leq r,s\leq 1, r+s\leq 1\}$. 
...
by setting 
\begin{equation*}
    V^G=(V^{G_1}\cup V^{G_2})/\langle v_i=w_i, 1\leq i\leq n\rangle \ ,
\end{equation*}
...
$1\leq i\leq m$ such that $v,v'\in V_i$. 
...
\begin{proof}
    ...
    , 1\leq j\leq N(\mathcal{T}) \right\rangle$$ 
    ...
    $1\leq j\leq p$ translates through $\mathcal{L}$. 
...
% =\gamma^{(i)},1\leq i\leq p+1).\label{eq:product_formula_volume}
\end{align*}
...
volume expression for the volume $M_{g,n}(\alpha_1,\ldots,\alpha_p)$ of flat $SU(n)$--connections on a surface $\mathcal{M}$ of genus $g$ with $p$ boundary components for $\alpha_1,\ldots,\alpha_p\in\mathcal{H}_{reg}$. 
...
given by
$$\mathcal{A}= \left\{t_1\geq\ldots\geq t_n \mid \sum_{i=1}^nt_i=0, \ t_1-t_n\leq 1 \right\} .$$
...
$dt=p_{R}^{*}d\ell_{\mathbb{R}^R}$ on $\mathcal{A}$. 
This volume form is independent of $R$ and denoted by $dt$ in the sequel. 
Choosing for example $R=\{1,\ldots n-1\}$, we have
\begin{align*}
Vol(\mathcal{A})=&\int_{\mathbb{R}^{n-1}}\indic_{t_1\geq\ldots\geq t_{n-1},\,t_1+\sum_{i=1}^{n-1}t_i\leq 1}\prod_{i=1}^{n-1} dt_i\\
=&\int_{\mathbb{R}^{n-1}}\indic_{1\geq u_1\geq\ldots\geq u_{n-1}\geq 0}\frac{1}{n}\prod_{i=1}^{n-1} du_i=\frac{1}{n!},
\end{align*}
where we did the change of variable $\phi:(t_i)_{1\leq i\leq n-1}\mapsto (t_i+\sum_{i=1}^{n-1}t_i)_{1\leq i\leq n-1}$ with $Jac(\phi(t))=n$.
...
\begin{align*}
\Vol(M(\mathcal{M},\alpha_1,\ldots,\alpha_p))=\frac{1}{k}\int_{\mathcal{A}}\Vol(M(\widehat{\mathcal{M}},\alpha_1,\ldots,\alpha_p,\phi(t),\phi(-t)))\Delta(t)^2dt,
\end{align*}
where $k=1$ if $\widehat{\mathcal{M}}$ is connected and $\#Z(SU(n))=n$ otherwise, $dt$ is the previous volume form on $\mathcal{A}$ and $\Delta(t)=2^{n(n-1)/2}\prod_{1\leq i<j\leq n}\sin\left(\pi(t_i-t_j)\right)^2$.
\end{theorem}
Note that we slightly adapted the result of \cite{Meinrenken_Woodward}, which was stated for the torsion volume of \cite{witten1992two}, to translate it in the symplectic picture : this only consists in adding the term $\Delta(t)$. Let us first remark that considering $U(n)$-valued connection instead of $SU(n)-$valued connection does not change the volume. Indeed, suppose that $\mathcal{M}$ corresponds to a surface of genus $g$ with $p$ boundary components. Then, for any $\alpha_1,\ldots,\alpha_{p}\in\mathcal{H}_{reg}$,
\begin{align*}
...
\begin{align*}
\Vol(M(\mathcal{M},\alpha_1,\ldots,\alpha_p))=\frac{1}{n}\int_{\mathcal{H}_{reg}^0}\Vol(M(\widehat{\mathcal{M}},\alpha_1,\ldots,\alpha_p,\theta,\tilde{\theta}))\Delta(\theta)^2d\theta,
\end{align*}
and, if $\widehat{\mathcal{M}}$ is connected,
\begin{align*}
\Vol(M(\mathcal{M},\alpha_1,\ldots,\alpha_p))=\int_{\mathcal{H}_{reg}}\Vol(M(\widehat{\mathcal{M}},\alpha_1,\ldots,\alpha_p,\theta,\tilde{\theta}))\Delta(\theta)^2d\theta,
\end{align*}
\end{proposition}
\begin{proof}
...
$$\Vol(M(\mathcal{M},\alpha_1,\ldots,\alpha_p))=\frac{1}{k}\int_{\mathcal{H}_{reg}^0}\Vol(M(\widehat{\mathcal{M}},\alpha_1,\ldots,\alpha_p,\theta,\widetilde{\theta}))\Delta(\theta)^2d\theta,$$
...
\begin{align*}
&\frac{1}{k}\int_{\mathcal{H}_{reg}^0}\Vol(M(\widehat{\mathcal{M}},\alpha_1,\ldots,\alpha_p,\theta,\widetilde{\theta}))\Delta(\theta)^2d\theta\\
&\hspace{2cm}=n\int_{0}^{1/n}\left(\int_{\mathcal{H}_{reg}^0}\Vol(M_{U(n)}(\widehat{\mathcal{M}},\alpha_1,\ldots,\alpha_p,t\cdot\theta,\widetilde{t\cdot\theta}))\Delta(\theta)^2d\theta\right) dt\\
&\hspace{2cm}=\int_{\mathcal{H}_{reg}}\Vol(M_{U(n)}(\widehat{\mathcal{M}},\alpha_1,\ldots,\alpha_p,u,\widetilde{u}))\Delta(u)^2du,
\end{align*}
...
\begin{align*}
Z_{g,p}(\alpha_1,\dots,\alpha_p)=Z_{g,p}(\hat{\alpha}_1,\dots,\hat{\alpha}_p)=&\int_{\mathcal{H}_{reg}}\Vol(M(\mathcal{M}',\hat{\alpha}_1,\ldots,\hat{\alpha}_p,\theta,\tilde{\theta}))\Delta(\theta)^2d\theta.\\
=&\int_{\mathcal{H}_{reg}}\Vol(M(\mathcal{M}',\alpha_1,\ldots,\alpha_p,\theta,\tilde{\theta}))\Delta(\theta)^2d\theta.
\end{align*}
Since $3(g-1)+p+2<N$, by induction
$$\Vol(M(\mathcal{M}',\alpha_1,\ldots,\alpha_p,\theta,\tilde{\theta}))=\frac{c_{g-1,p+2}}{\prod_{j=1}^p\Delta(\alpha_j)}\sum_{\substack{G \in \mathcal{G}^{(g,p)}}}
        \Vol \left[ \honey^G(\alpha_1, 
        \dots, \alpha_p,\theta,\tilde{\theta}) \right],$$
and thus by Proposition \ref{prop:formula_contracting_honey},
\begin{align*}
Z_{g,p}(\alpha_1,\dots,\alpha_p)=&\frac{c_{g-1,p+2}}{\prod_{j=1}^{-2}p\Delta(\alpha_j)}\int_{\mathcal{H}_{reg}}\sum_{\substack{G \in \mathcal{G}^{(g-1,p+2)}}}
        \Vol \left[ \honey^G(\alpha_1, 
        \dots, \alpha_p,\theta,\tilde{\theta})\right]\frac{\Delta(\theta)^2}{\Delta(\theta)\Delta(\tilde{\theta})}d\theta\\
        =&c_{g-1,p+2}\sum_{\substack{G \in \mathcal{G}^{(g,p)}}}
        \Vol \left[ \honey^G(\alpha_1, 
        \dots, \alpha_p) \right],
 \end{align*}
where we used the fact that $\Delta(\theta)=\Delta(\tilde{\theta})$ on the second equality. In the second case, $\mathcal{T}'$ is obtained by gluing a  a surface $\mathcal{M}'$ with genus $g$ and $p-1$ points removed and a triangle $T$. Let $\widehat{M}=\mathcal{M'}\cup T$ be the corresponding disconnected surface. Then, by \eqref{eq:SU_n_equal_Un} and Proposition \ref{prop:volume_formula_flat},
\begin{align*}
...
\end{align*}
By induction,
$$\Vol(M(\mathcal{M}',\alpha_1,\ldots,\alpha_{p-2},\theta,))=\frac{c_{g,p-1}}{\Delta(\theta)\prod_{i=1}^{p-2}\Delta(\alpha_i)}\sum_{\substack{G \in \mathcal{G}^{(g,p-1)}}}
        \Vol \left[ \honey^G(\alpha_1, 
        \dots, \alpha_{p-2},\theta) \right],$$
 and
 $$\Vol(M(T,\theta,\alpha_{p-1},\alpha_p))=\frac{c_{0,3}}{\Delta(\theta)\Delta(\alpha_{p-1}),\Delta(\alpha_{p})}\sum_{\substack{G \in \mathcal{G}^{(g,p-1)}}}
