...
\providecommand{\indic}{\mathbbm 1}\DeclareMathOperator{\interior}{int}
...
Let $\mathcal{M}=(M_1\ldots, M_{2g+p-2})$ be a decomposition in pants
of this surface. 
...
{\color{darkgreen}For $G = (V, E) \in\mathcal{G}^{(p, g)}$, there is a natural parametrization of 
...
{\color{blue} La section "Volume formula" n'a plus raison d'être} 
...
$\mathcal{O}_{\alpha_1},\mathcal{O}_{\alpha_2}\ldots,\mathcal{O}_{\alpha_p}$. For $g,p\geq 0$, Set $d\Vol\in\Omega^{n_{g,p}}(\mathbb{R}^{E^{(g,p)}})$ for the volume form on $n_{g,p}$-dimensional subspaces induced by the canonical Riemannian metric on $\mathbb{R}^{E^{(g,p)}}$. Hence, 
$$d\Vol=\sum_{1\leq i_1<\ldots<i_{n_{g,p}}<\frac{3Nn(n+1)}{2}}dx_{i_1}\wedge\ldots \wedge dx_{i_{n_{g,p}}}.$$
In the following statement, we write $\Vol(K)$ for the integration of $d\Vol$ on a $n_{g,p}$-dimensional submanifold and we let $\Delta(x)=\prod_{i<j}(x_j-x_i)$ denote the Vandermonde determinant.
...
for the marginal Yang--Mills partition function of an
oriented surface 
...
\begin{equation}
	...
	\mathcal{F}(\phi)=\left\{\lambda\in\Omega^1(G),\sum_{x \sim v} \lambda (v,x) = \phi(v)\right\}.
\end{equation}
...
\end{proof}
\begin{proposition}\label{prop:divergence_volume}
...
\begin{equation}\label{eq:definition_volume_form}
d\Vol=\frac{1}{\sqrt{\# \mbox{ Spanning trees of } G}} \mathcal Leb_{\mathcal F(\phi)},
\end{equation}
so that $(\phi_S)_{*}d\Vol=\mathcal Leb_{\Omega^1(S)}$.


\subsection*{Boundary and sieving of graphs}

\label{subsec:sieving_contraction}
For $G$ a finite graph, let us denote by $\partial G$ (resp. $\interior(G)$) the set vertices of $G$ of degree $1$ (resp. degree larger than $1$) and by $\partial E$ the set of edges adjacent to $\partial G$. If $\phi\in \Omega ^0(G)$, we denote by $\partial \phi$ the restriction of $\phi$ to $\partial G$. Likewise, we denote by $\partial \lambda$ the restriction of $\lambda\in \Omega^1(G)$ to $\{(a,b),\{a,b\}\in\partial E\}$. By the previous section, for $R\subset  V$, $\mathcal{F}(\phi)$ is non-empty if and only if 
$$\sum_{v\in R}\phi(v)=-\sum_{v\in V\setminus R}\phi(v).$$


\begin{definition}[Sieving of graphs]\label{def:sieving}
Let $r\geq 1$. Let $G=(V,E)$ be a finite graph (non necessarily connected) and $W,W'\subset \partial G$ with a bijection $g:W\rightarrow W'$. The sieving of $G$ along $(R_1,R_2)$ is the graph $G_{W*W'}=(\tilde{V},\tilde{E})$ obtained as follows:
\begin{itemize}
...
\end{itemize}
\end{definition}
This construction informally amounts to merge $v$ and $g(v)$ for $v\in W$ and considering the resulting edge structure inferred by $E$. Since $W,W'\subset \partial G$, $\vert \tilde{E}\vert=\vert E\vert$ as long as each connected component of $G$ has a size at least $3$ (which will always by the case in the present paper). There is moreover a canonical bijection between $E$ and $\tilde{E}$. Let us closely look at the behavior of the equation \eqref{eq:DivOfLambda} with respect to the sieving of graphs. 

In the following, if $G=(V,E)$ is a finite graph, $V=S_1\sqcup\ldots \sqcup S_r$ is a partition of $V$ and $\phi\in \Omega^0(G)$, we write by abuse of notation $(\phi_{\vert S_1},\ldots,\phi_{\vert S_r})$ instead of $\phi$ to detail the decomposition of $\phi$ along this partition. Moreover, we write $\mathcal{F}_{G}(\phi)$ instead of $\mathcal{F}(\phi)$ to emphasize that the equation \eqref{eq:DivOfLambda} is considered in the graph $G$.
\begin{proposition}[Product formula]\label{prop:divergence_product}
Let $G_1=(V_1,E_1),G_2=(V_2,E_2)$ be two connected finite graphs, $W\subset \partial G_1,W'\subset G_2$ and $g:W\rightarrow W'$ a bijection, and set $\tilde{G}=(G_1\cup G_2)_{W*W'}$. Let $\phi\in \Omega^0(\tilde{G})$ be such that $\sum_{v\in \tilde{V}}\phi(v)=0$:
\begin{enumerate}
\item $\lambda\in\mathcal{F}_{\tilde{G}}(\phi)$ if and only if $\lambda_{\vert E_1}\in \mathcal{F}_{G_1}(\phi_{\vert V_1\setminus W},(x_w)_{w\in W})$ and $\lambda_{\vert E_2}\in\mathcal{F}_{G_2}(\phi_{\vert V_2\setminus W'},(\phi(\tilde{w})-x_{g^{-1}(w)})_{w\in W'})$ for some $x_w,w\in W$ such that $\sum_{v\in V_1\setminus W}\phi(v)+\sum_{w\in W}x_w=0$.
\item let $w_0\in W$. For $K_1\subset \Omega^2(G_1), K_2\subset \Omega^{2}(G_2)$,
\begin{align*}
\Vol_{\tilde{G}}((K_1\times K_2)\cap\mathcal{F}_{\tilde{G}}(\phi))=&\int_{\mathbb{R}^{\vert W\vert-1}}\Vol_{G_1}\left[K_1\cap \mathcal{F}_{G_1}(\phi_{\vert V_1},(x_w)_{w\in W\setminus W_0},y(x)))\right]\\
&\hspace{2cm}\cdot \Vol_{G_2}\left[K_2\cap \mathcal{F}_{G_2}(\phi_{\vert V_2},(\phi(\tilde{w})-x_w)_{w\in W\setminus \{w_0\}}),\phi(\tilde{w})-y(w))\right]dx,
\end{align*}
where $y(w)$ is the unique solution to $\sum_{v\in V_1\setminus W}+\sum_{w\in W\setminus \{w_0\}}x_w+y(w)=0$.

\end{enumerate}
\end{proposition}
\begin{proof}

(1) Remark that $\lambda\in\mathcal{F}_{\tilde{G}}(\phi)$ if and only if
$$\sum_{e,\bar{e}=x}\lambda(e)=\phi(x)$$
for all $x\in \tilde{V}$. 

Let $\lambda\in \mathcal{F}_{\tilde{G}}(\phi)$. Set $\tilde{\phi}(x)=\phi(\tilde{x})$ for $x\in V_1\setminus W$ and, for $w\in W$, $x_w=\lambda(\tilde{e})$ where $\tilde{e}$ is the unique edge of $\tilde{G}$ starting from $\tilde{w}$ and ending on $V_1$ (recall that $W_1\subset \partial G_1$). Then, since $\lambda\in \mathcal{F}_{\tilde{G}}(\phi)$, on has 
$$\lambda_{\vert E_1}\in \mathcal{F}(\phi_{V_1\setminus W_1},(x_w)_{w\in W_1}).$$
 By the divergence condition, one must have 
$$\sum_{v\in V_1}\phi(v)+\sum_{w\in W_1}x_w=0.$$
Similarly, $\lambda_{\vert E_2}\in \mathcal{F}(\phi_{V_2\setminus W'},(y_w)_{w\in W_2})$ with $y_w=\lambda(e)$, where $e$ is the unique edge of $\tilde{E}$ starting from $w$ and ending on $V_2$. By using the divergence condition on $\tilde{w}=\{w,g(w)\}$ for $w\in W$, one has $\phi(\tilde{w})=x_w+y_{g(w)}$, and thus 
$$y_{g(w)}=\phi(\tilde{w})-x_w.$$
Reciprocally, one check that if $\lambda\in \Omega^1(\tilde{G})$ is such that $\lambda_{\vert E_1}\in \mathcal{F}(\phi_{V_1\setminus W_1},(x_w)_{w\in W_1})$ and $\lambda_{\vert E_2}\in \mathcal{F}(\phi_{V_2\setminus W'},(\phi(\tilde{w})-x_{g^{-1}(w)})_{w\in W_1})$, then $\lambda\in \mathcal{F}(\tilde{\phi})$.

(3) Set $s+1=\vert W\vert=\vert W'\vert$ and write $W=\{w_0,\ldots,w_s\}$. Let $e_i$ be the edge of $E_1$ adjacent to $w_i$. Let $T_i$ be a spanning tree of $G_i$ for $i\in \{1,2\}$. Since each edge $\tilde{w},w\in W$ is bivalent in $\tilde{G}$, removing the edges $e_j$, $1\leq j\leq s$ from $T_1\cup T_2$ yields a spanning tree $T$ of $\tilde{G}$. By Proposition \ref{prop:BijectionTrees} applied to $S=T^c$, the restriction map $\phi_{S}$ is a bijection from $\mathcal{F}(\phi)$ to $\Omega_1(S)$. Moreover, by Proposition \ref{prop:divergence_volume_form}, $(\varphi_S)_*d\Vol=\mathcal Leb_{\Omega^1(S)}$. Let us write $S_i=E_{i}\setminus T_i$ and $R=\{e_1,\ldots,e_{s}\}$. Then, 
$$T^c=[E_{1}\setminus  T_1]\cup [E_2\setminus T_2]\cup R=S_1\cup S_2\cup R.$$
Let $K_1\subset \mathbb{R}^{E_1}$ and $K_2\subset \mathbb{R}^{E_2}$. Then, for $(t_1,t_2,x)\in\mathbb{R}^{S_1}\times \mathbb{R}^{S_2}\times \mathbb{R}^{s-1}$, 
$\varphi_{T^c}^{-1}(t_1,t_2,x)\in\mathcal{F}(\phi)$. By the previous statement, this is equivalent to the fact that $\varphi_{E_1}(\varphi_{T^c}^{-1}(t_1,t_2,x))\in\mathcal{F}(\phi_{\vert V_1\setminus W},(x_w)_{w\in R},y)$ where $y$ is the unique solution to $\sum_{v\in V_1\setminus R}\phi(v)+\sum_{w\in R}x_w+y=0$, and $\varphi_{E_2}(\varphi_{T^c}^{-1}(t_1,t_2,x))\in\mathcal{F}(\phi_{\vert V_2\setminus W'},(\phi(\tilde{w})-x_{w}))_{w\in R},\phi(w_0)-y)$. Let $\varphi^x_{S_1}$ be the projection from $\mathcal{F}(\phi_{\vert V_1\setminus W},(x_w)_{w\in R},y))$ to $\Omega^2(S_1)$ and $\varphi^x_{S_2}$ be the projection from $\mathcal{F}(\phi_{\vert V_2\setminus W'},(\phi(\tilde{w})-x_{w}))_{w\in R},\phi(w_0)-y)$ to $\Omega^2(S_2)$. Since $E_i\setminus S_i$ is a spanning tree of $G_i$, each map $\varphi^x_{S_i}$ is bijective. Moreover, if $\varphi_{E_1}(\varphi_{T^c}^{-1}(t_1,t_2,x))\in\mathcal{F}(\phi_{\vert V_1\setminus W},(x_w)_{w\in R},y)$, since then $\varphi^x_{S_1}\circ\varphi_{E_1}(\varphi_{T^c}^{-1}(t_1,t_2,x))$ is well-defined and equal to $t_1$, we have 
$$\varphi_{E_1}(\varphi_{T^c}^{-1}(t_1,t_2,x))=(\varphi^x_{S_1})^{-1}(t_1).$$
Likewise, if $\varphi_{E_2}(\varphi_{T^c}^{-1}(t_1,t_2,x))\in\mathcal{F}(\phi_{\vert V_2\setminus W'},(\phi(\tilde{w})-x_{w}))_{w\in R},\phi(w_0)-y)$, then
$$\varphi_{E_2}(\varphi_{T^c}^{-1}(t_1,t_2,x))=  (\varphi^x_{S_1})^{-1}(t_2).$$ 
Hence,
$\varphi_{T^c}^{-1}(t_1,t_2,x)\in K_1\times K_2\cap \mathcal{F}(\phi)$ if and only 
$(\varphi^x_{S_1})^{-1}(t_1)\in K_1$ and $(\varphi^x_{S_1})^{-1}(t_2)\in K_2$. 


Hence,
\begin{align*}
Vol(K_1\times K_2\cap\mathcal{F}(\phi))=&\int_{\mathbb{R}^{S_1}\times \mathbb{R}^{S_2}\times \mathbb{R}^{s}}\mathbf{1}_{\varphi_{T^c}^{-1}(t_1,t_2,x)\in K_1\times K_2}dt_1dt_2dx\\
=&\int_{\mathbb{R}^s}\left(\int_{\mathbb{R}^{S_1}\times \mathbb{R}^{S_2}}\mathbf{1}_{(\varphi_{S_1}^x)^{-1}(t_1)\in K_1}\mathbf{1}_{(\varphi_{S_2}^x)^{-1}(t_2)\in K_2}dt_1dt_2\right)dx\\
=&\int_{\mathbb{R}^{s}}\left(\int_{\mathbb{R}^{S_1}}\mathbf{1}_{(\varphi_{S_1}^x)^{-1}(t_1)\in K_1}dt_1\right)\cdot\left( \int_{\mathbb{R}^{S_2}}\mathbf{1}_{(\varphi_{S_2}^x)^{-1}(t_2)\in K_2}dt_2\right)dx\\
=&\int_{\mathbb{R}^{s}}\Vol\left[K_1\cap \mathcal{F}(\phi_{\vert V_1\setminus W},(x_w)_{w\in R},y)\right]\\
&\hspace{3cm}\cdot \Vol\left[K_2\cap\mathcal{F}(\phi_{\vert V_2\setminus W'},(\phi(\tilde{w})-x_{w}))_{w\in R},\phi(w_0)-y)\right]dx.
\end{align*}


\end{proof}

There exists a similar formula when a gra
...
\begin{align*}
Vol(K\cap\mathcal{F}_{\tilde{G}}(\phi))=&\int_{\mathbb{R}^{\vert W\vert-1}}\Vol\left[K\cap \mathcal{F}_{G}(\phi_{\vert V\setminus (W\cup W')},(x_w)_{w\in W},(\phi(\tilde{w})-x_{g^{-1}(w')})_{w\in W'})\right]dx.
\end{align*}
\end{proposition}
\begin{proof}
...
\begin{proof}
    ...
    only depends on the type and color map of the boundary edges and on the order on the boundary vertices}.
    \\
    \\
...
Let us provide a description of honeycombs with structure graph $G$ in terms of flows. Suppose that $G\in \mathcal{G}_d$. 
    By the condition (1) Definition \ref{def:triangular_honey}, 
    $G$ has only vertices of degree $1$ or $3$ and thus $v$ has three adjacent 
    edges $e_\ell,\, \ell\in \{0,1,2\}$.  Dy $\interior(G)$ the set of vertices of degrees $3$ and let $v\in\interior(G)$. By (2) of Definition \ref{def:toric_honeycomb}, 
    ...
    $e_\ell \subset \left\{v+s(v)\ed^{2(\ell+1)i\pi/3} \mathbb{R}_{\geq 0} \right\}$. Since any univalent vertex $v\in\partial G$ is connected to a trivalent vertex, such a $s(v)\in \{-1,1\}$ exists in fact for any $v\in V$.
    \\
    \\
    Let $v,v'\in   V$ be such that $e=\{v,v'\}$. 
    By the previous reasoning, there exist $\ell,\ell'$ such that 
    $e\subset \left\{v+s(v)\ed^{2\ell i\pi/3} \mathbb{R}_{\geq 0} \right\}$ and 
    $e\subset \left\{v'+s(v')\ed^{2\ell'i\pi/3} \mathbb{R}_{\geq 0} \right\}$.  
    Necessarily, $\ell=\ell'$ and $s(v)=-s(v')$. In particular, the map $s:v\mapsto s(v)$ only depends on the value of $s$ on the boundary $\partial G$. Since $s(v)$ for $v\in \partial G$ is given by the type and thus the color of the unique adjacent edge, see Definition \ref{def:triangular_honey}.(3), we deduce by Lemma \ref{lem:types_colors_determined_by_boundaries} that $s$ only depend on $G$.  In the following definition, recall the definition of the height of an edge in Definition \ref{def:triangular_honey}.
    
 \begin{definition}[Flow of a honeycomb]
    \label{def:flow_honeycomb}
    The \textit{flow} of a honeycomb 
    $h\in\honey_{n,d}^G$ is the map $\mathcal{L}[h] \in \Omega^1(G)$   which to an oriented edge $(v,v')$ with $s(v)=1$ and $s(v')=-1$  associates the height of the non-oriented edge, that is, $\mathcal{L}[h](v,v') = L(\{v,v'\})$.
\end{definition}


\noindent
If $v\in \interior(G)$, $v$ corresponds to a point if $T$ and thus its coordinates $(v_1,v_2,v_3)$ satisfies $v_1+v_2+v_3=1$. If $e_1,e_2,e_3$ are the three edges adjacent to $v$, we thus have by definition of $L(e_i),1\leq i\leq 3$ (see Definition \ref{def:triangular_honey}(2))
$$L(e_1)+L(e_2)+L(e_3)=1.$$
Hence, considering now oriented edges yields 
\begin{equation}\label{eq:honeycomb_divergence_1}
\sum_{v\sim v'}\mathcal{L}[h](v,v')=s(v)
\end{equation}
for $v\in \interior(G)$.
...
$\partial E = \left\{ \{v, v'\} \mid v \in \partial V \right\}$ consists of edges having a 
vertex of degree one. Since by Definition \ref{def:triangular_honey}, $\partial G \subset \partial T$, 
we may write $\partial E = \left( e_1, \dots, e_{3n} \right)$, where for $0 \leqslant l \leqslant 2$
...
given by 
$$ g^{\alpha,\beta,\gamma}_{\ell n+i}=\alpha_i-c(e_{\ell n+i})=
\left\lbrace 
\begin{aligned} 
    &\alpha_i&\text{if }c(e_{\ell n+i})=0 \\ 
    &\alpha_i-1&\text{if }c(e_{\ell n+i})=1
\end{aligned} \right. $$
for $\ell = 0$ and $1\leq i\leq n$ and replacing $\alpha_i$ by $\beta_i$ (resp. $\gamma_i$) 
when $\ell = 1$ (resp. $\ell = 2$). Remark that for $v\in \partial G$, $s(v)=1$ if and only if $c(e)=1$, where $e$ is the unique edge adjacent to $v$, see for example Figure \ref{fig:boundary_toric_honey}. Hence, the boundary condition translates into the condition 
\begin{equation}
    \label{eq:honeycomb_divergence_2}\mathcal{L}[h](v_{\ell n+i},v')=g^{\alpha,\beta,\gamma}_{\ell n+i},
\end{equation}
where $v'$ is the unique vertex of $V$ adjacent to $v_{\ell n+i}$. Hence, setting $\phi_{\alpha,\beta,\gamma}(v)=s(v)$ for $v\in \interior(G)$ and $\phi_{\alpha,\beta,\gamma}(v_{\ell n+i})=g^{\alpha,\beta,\gamma}_{\ell n+i}$ for $v^{\ell n+i}\in \partial G$, \eqref{eq:honeycomb_divergence_1} and \eqref{eq:honeycomb_divergence_2} yields that 
$$\mathcal{L}(h)\in \mathcal{F}_{G}(\phi_{\alpha,\beta,\gamma})$$
for $h\in \honey_{n,d}^G(\alpha,\beta,\gamma)$.
...
\begin{proposition}[Honeycomb injection]
    ...
    since $L$ is for edge height)} is an injective map from $\honey^G$ to $\Omega^1(G)$ and 
...
The fact that $\mathcal{L} \left(\honey_{n,d}^G(\alpha,\beta,\gamma)\right) 
    \subset  \mathcal{F}_{G}(\phi_{\alpha,\beta,\gamma})$ is given by the previous discussion and remains to prove the injectivity of the map. Let $h_1,h_2\in \honey^G$ such that $\mathcal{L}[h_1] = \mathcal{L}[h_2]$. 
...
\\
\\
Suppose that $v\in \interior(G)$. By Definition \ref{def:triangular_honey}, $v$ is a trivalent vertex 
...
$e=(v,v+e^{\pi i\ell/3})$ {\color{blue} isn't it $e=(v,v - e^{2\pi i\ell/3})$ ?} with $\ell\in\{0,1,2\}$ is 
...
\begin{itemize}
\item if $v\in \interior(G)^{\Gamma}$, then $v$ is adjacent to three edges 
...
\begin{lemma}[Edge geodesics]
    ...
    % either $c(e) \neq m$ and
    % $$\Phi(e) \subset x_v + \ed^{2(\ell+1)\pi i/3} \mathbb{R}_{>0} \ ,$$
    % or $c(e)=m$ and 
    % $$\Phi(e)=\{x_v\} \ .$$}
    ...
    Then, either $c(e) \neq m$ and
    $$\Phi(e)\subset x_v + \epsilon\mathbb{R}_{>0} e^{2 \pi i(\ell+1)/3} ,$$
    or $c(e)=m$ and 
    $$\Phi(e)=\{x_v\} .$$
\end{lemma}
...
XXX Attention : conflit avec les projections sur les coordonnées...XXX
...
$\phi_{\alpha,\beta,\gamma}:V\rightarrow \mathbb{R}$ has been defined before Proposition \ref{prop:parametrization_triangular_honey_first}
...
Let $\hat{e}=\{v,v'\}\in E^C$ of type $\ell$ with $s(v)=1$ and $s(v')=-1$, and suppose without loss of generality that $v_{\ell}<v'_{\ell}$. 
...
$L(\hat{\Phi}(\hat{e}))=(x_v)_{\ell}=L(\mathcal{E}(\hat{e}))$. 
{\color{blue} $\mathcal{L}[p_c[H]](\hat{\Phi}[H](\hat{e})) = (x_v)_{\ell} = L(\mathcal{E}(\hat{e}))$ ?}
Hence, $\mathcal{L} \circ p_{C}$ is the restriction of the linear map from $\mathbb{R}^{E_{n,d}}$ to $\Omega^1(G^C)$ 
mapping $(x(e))_{e\in E_{n,d}}$ to $\sum_{\hat{e}\in E^C}x(\mathcal{E}(\hat{e}))\delta_{\vec{e}}$, where for $e=\{v,v'\}\in E^C$ with $s(v)=1$ and $s(v')=-1$, $\vec{e}=(v,v')$. Remark that this map has integer coefficients in the canonical bases of both vector spaces.
\\
\\
...
$h(\tilde{e}) = v_\ell$ where $e = (v, v - \ed^{2 \pi i \ell / 3})$. Since $x_\ell = v_\ell$, one 
...
$\{w,v'\}$ is an edge of $E^C$ of type $\ell+1$ and $v'_{\ell-1}>w_{\ell-1}$. This cover relation translates into a cover relation in $\vec{E}^C$ by saying that $(v,w)<(v',w')$ is and only if $\{v,w\}<\{v',w'\}$ in the former sense.

\begin{corollary}[Parametrization of labels]\label{cor:paramietrization_label}
    For a color map $C$ and $\Gamma^C\in \mathcal{G}_d$,
    $$\mathcal{L} \left(\honey_{n,d}^{\Gamma^C}(\alpha,\beta,\gamma) \right) 
    = \mathcal{F}_{\Gamma^C}(\phi_{\alpha,\beta,\gamma})\cap K^C,$$
    where $K^C\subset \Omega^1(G^C)$ is the cone defined as 
    $$K^C = \left\{(\omega \in \Omega^1(G^C) \mid \vert \omega(e)\vert<\vert \omega(e')\vert\,\text{ if }e<e'\, e,e'\in\vec{E}^C\right\}.$$ 
\end{corollary}

\begin{proof}
    By Proposition \ref{prop:honey_hive}, $\mathcal{L} \left(\honey_{n,d}^{\Gamma^C}(\alpha,\beta,\gamma) \right) 
    = \Phi^{-1} \left(\dualhive^C(\alpha,\beta,\gamma)\right)$, with, for $\omega\in \Omega^1(G)$ and $e\in E_{n,d}$, 
    $$\Phi[\omega](e)=\left\lbrace\begin{aligned} \vert \omega(\vec{\hat{e}})\vert&\text{ if }e\in \hat{e}, c(e)\not=m\\
    1-\vert \omega(\vec{\hat{e_1}})\vert-\vert \omega(\vec{\hat{e_2}})\vert&\text{ if }c(e)=m,\,e_1\in\hat{e}_1,e_2\in\hat{e}_2 ,\,(e_1,e_2,e)\text{ triangular face of }H_{n,d}\end{aligned}\right..$$
    ...
    One then checks that $\Phi^{-1}(\mathcal{H}^C(\alpha,\beta,\gamma))=\mathcal{F}_{G^C}(\alpha,\beta,\gamma)$ 
...
Using the results of this section, we present a reformulation of this results in terms of triangular honeycombs. In the following, 
$$\Vol(\left[\honey_{n,d}^G(\alpha,\beta,\widetilde{\gamma})\right]):=\Vol(\mathcal{L}\left[\honey_{n,d}^G(\alpha,\beta,\widetilde{\gamma})\right]),$$
where $\mathcal{L}$ is the map from $\honey_{n,d}^G(\alpha,\beta,\widetilde{\gamma})$ to $\mathcal{F}(\phi_{\alpha,\beta,\gamma})$ defined in Definition \ref{def:flow_honeycomb} and $d\Vol$ is the volume form defined in \eqref{eq:def_volume}.
...
Let $C$ be a color map. By Proposition \ref{prop:BijectionTrees}, there exists a set $R$ such the restriction map $\varphi_{R}:\mathcal{F}_{G^C}(\phi_{\alpha,\beta,\gamma}))\rightarrow \Omega^1(S)$
 is a bijection with integral coefficient in the canonical bases. 
Denote by $i_{R}:\Omega^1(R)\rightarrow \Omega^1(G)$ the corresponding inverse map, which has thus affine with integer 
coefficients in the canonical basis and is a bijection from $\Omega^1(R)$ to $\mathcal{F}_{G^C}(\phi_{\alpha,\beta,\gamma}))$. 
Since, by Proposition \ref{prop:honey_hive} and Corollary \ref{cor:paramietrization_label}, $\Phi^C:\Omega^1(G^C)\rightarrow \mathbb{E}^{E_{n,d}}$ is an affine integral map 
...
$$p_{S}\circ Rot[C\rightarrow C_0]\circ \Phi^C\circ i_R: \Omega^1(R) \rightarrow \R^S$$
...
$\dualhive^{C_0}(\alpha,\beta,\gamma)$ to $\mathcal{F}_{G^{C_0}}(\phi_{\alpha,\beta,\gamma})$,
$$ \left(p_{S}\circ Rot[C\rightarrow C_0]\circ \Phi^C\circ i_R \right)^{-1} 
= \varphi_{R}\circ L\circ p_{C_0}\circ Rot[C_0\rightarrow C] \circ i_S:\mathbb{R}^S\rightarrow \Omega^1(R)$$
...
{\color{blue} its determinant as an isomorphism from $\Omega^1(R)$ to $\R^S$ has modulus one.}
Hence,
\begin{align*}
\Vol_S\left[Rot[C\rightarrow C_0] \left(\dualhive^C(\alpha,\beta,\gamma)\right) \right] = 
& Leb\left[u\in\mathbb{R}^S,Rot[C_0\rightarrow C]\circ i_{S}(u)\in \left(\dualhive^C(\alpha,\beta,\gamma)\right) \right]
\\
=& Leb\left[u\in\mathbb{R}^S,\varphi_{R}\circ \mathcal{L}\circ p_{C}\circ Rot[C_0\rightarrow C]\circ i_{S}(u)\in \varphi_R \left(\honey_{n,d}^C(\alpha,\beta,\gamma)\right) \right]\\
=& Leb\left[z\in\Omega^1(R),z\in \varphi_R\left(\honey_{n,d}^C(\alpha,\beta,\gamma)\right) \right]\\
...
Let $\mathcal{T}$ be an oriented surface with boundary obtained by gluing $m$ equilateral triangles $T^1,\ldots,T^m$ along their boundaries, such that $p$ edges $L_1,\ldots,L_{p}$ of equilateral triangles are not glued.  Each edge $L_j$ has a natural orientation $\ell_j$ coming from the equilateral it belongs to. Suppose that $h$ is a honeycomb on $\mathcal{T}$ such that each $h\cap \mathcal{T}^i$ is triangular. Then, the structure graph $\hat{G}[h]$ of $h$ as defined after Definition \ref{def:toric_honeycomb} has $pn$ univalent vertices, $n$ of them being on each boundary component $L_j,\, 1\leq j\leq p$, and $\frac{(3m-p)n}{2}$ bivalent vertices, $n$ of them being on a the boundary of a common triangle while being in the interior of the surface. 

 Denote by $v^j_m,\,1\leq m\leq n$ the univalent vertices corresponding to the point on $L_{j}$ ranked decreasingly following their $(\ell_j+1)$-coordinate. Let $\mathcal{G}^{(p, g)}$ be the set of structure graphs appearing in $\honey_{\mathcal{T}}$ rooted at $v^1_1$.

Let us introduce some notations. First, for $1\leq j\leq p$, let $s_j,\ell_j$be such that $L_j=\partial_{\ell_j}T^{s_j}$. Then, set $N(\mathcal{T})=\frac{3m-p}{2}$ and denote by $B_{1},\ldots,B_{N(\mathcal{T})}$ the segment of $\mathcal{T}$ corresponding to boundary of triangles pasted together. Each $B_j$ is then adjacent to two triangle $T^{r^{(1)}_j}$ and $T^{r^{(2)}_j}$ and there exists $\ell^{(1)}_j,\ell^{(2)}_j$ such that $B_j=\partial_{\ell^{(1)}_j}T^{r^{(1)}_j}=\partial_{\ell^{(2)}_j}T^{r^{(2)}_j}$.


XXXConflict of notation i with injection edge/geodesicXXX
...
$$i:\left\lbrace\begin{aligned}&\honey_{\mathcal{T}'}&\rightarrow&\{(h^1,\ldots,h^m)\in\honey_{T^{1}}\times\ldots\times \honey_{T^m},h^{r_j}_{\vert B_j }=h^{r'_j}_{\vert B_j},1\leq j\leq N(\mathcal{T})\}\\
&h&\mapsto&(h\cap T^{i})_{1\leq i\leq m}\end{aligned}\right.$$
is bijective and restricts for each $G\in\mathcal{G}_{\mathcal{T}}$ and $(G_1,\ldots,G_m)=t(G)$ to a bijection 
$$i:\honey_{\mathcal{T}}^G\rightarrow \{(h^1,\ldots,h^m)\in\honey_{T^{1}}^{G_1}\times\ldots\times \honey_{T^m}^{G_m},h^{r_j}_{\vert B_j }=h^{r'_j}_{\vert B_j},1\leq j\leq N(\mathcal{T})\}.$$
\end{lemma}
\begin{proof}
The proof is done by induction on $m$. The result holds for $m=1$. Suppose the result true for $m\geq 1$, and let $\mathcal{T}$ be a surface obtained by pasting $m$ triangles $T^1,\ldots,T^m$ along some boundaries and that $L_1,\ldots,L_p$ are the boundary which have not been pasted together, with $m\geq 1$. Assume without loss of generality that $L_p$ belongs to $T^m$ and let $T^{m+1}$ is another oriented equilateral triangle and let $\mathcal{T}'$ be the surface obtained by gluing $T^{m+1}$ to $T^m$ along $\partial_0T^{m+1}$ and $L^1$. 
...
Hence, there is a map $\tilde{t}:\mathcal{G}_{\mathcal{T}'}\mapsto \mathcal{G}_{\mathcal{T}}\times \mathcal{G}$ and a map
$$\tilde{i}:\honey_{\mathcal{T}'}\rightarrow \honey_{\mathcal{T}}\times\honey_{n}$$
such that, for $G\in\mathcal{G}_{\mathcal{T}'}$ with $\tilde{t}(G)=(G_1,G_2)$, 
$$\tilde{i}(\honey_{\mathcal{T}'}^G)\subset\{(h^1,h^2)\in\honey_{T^{m+1}}^{G_1}\times \honey_{\mathcal{T}}^{G_2},h^1_{\vert \partial_0T^{m+1} }=h^2_{\vert L_p}\}.$$
If $h,h'\in \honey_{\mathcal{T}'}$ are such that $\tilde{i}(h)=\tilde{i}(h')=(h^1,h^2)$, then
$$h=h^1\cup h^2=h'$$
Hence, the map $i$ is injective. 

Reciprocally, let $(h^1,h^2)\in\honey_{T^{m+1}}^{G_1}\times \honey_{\mathcal{T}}^{G_2}$ be such that $h^1_{\vert \partial_0T^{m+1} }=h^2_{\vert L_p}$. Suppose without loss of generality that $T^{m+1}=\{-r+se^{2\pi i/3},\,0\leq r,s\leq 1, r+s\leq 1\}$ and that $\partial_0T^{m+1}=\{se^{2\pi i/3}\}=L_p$, so that $T^{m}=\{re^{\pi i/3}+se^{2\pi i/3},\,0\leq r,s\leq 1, r+s\leq 1\}$. Let $x\in h^1_{\partial_0T^{m+1}}=h^{2}_{\vert L_p}$. Then, by Condition (3) of Definition \ref{def:triangular_honey} (see Figure \ref{fig:common_boundary}): 
\begin{itemize}
\item a geodesic $e$ of $h^1$ arriving at $\partial_0T^{m+1}$ at $x$ is either colored $c(e)=0$ and included in $x+\mathbb{R}_{>0}e^{-2\pi i/3}$ or colored $c(e)=1$ and included in $x+\mathbb{R}_{<0}$. 
\item a geodesic $e'$ of $h^2$ arriving at $\partial_0T^{m+1})L_p$ at $x$ is either colored $c(e)=0$ and included in $x+\mathbb{R}_{<0}e^{-2\pi i/3}$ or colored $c(e)=1$ and included in $x+\mathbb{R}_{>0}$.
\end{itemize}
...
$$h=h_1\cup h_2$$ 
defines a honeycomb on $\mathcal{T}'$. Moreover, the structure graph $G$ of $h$ is obtained as follows : $V^G=(V^{G_1}\cup V^{G_2})/\langle v_i=w_i, 1\leq i\leq n\rangle$, where $w_1,\ldots,w_n$ are the $n$ first boundary vertices of $G_2$ (recall that $G_2$ is a colored graph with an order on the boundary vertices), and $E^G=E^{G_1}\cup E^{G_2}$.
In particular, the map $\tilde{t}:\mathcal{G}_{\mathcal{T}'}\rightarrow \mathcal{G}_{\mathcal{T}}\times \mathcal{G}$ is injective, and each restricted map 
$$\tilde{i}:\honey_{\mathcal{T}'}^G\rightarrow\{(h^1,h^2)\in\honey_{T^{m+1}}^{G_1}\times \honey_{\mathcal{T}}^{G_2},h^1_{\vert \partial_0T^{m+1} }=h^2_{\vert L_p}\}.$$
is bijective. 

Let $t_m,i_m$ be the maps given by induction on $\mathcal{G}_{\mathcal{T}}$ and $\honey_{\mathcal{T}}$. Then, the maps $(t_m\times \Id_{\mathcal{G}})\circ \tilde{t}$ and $(i_m\times \Id_{\honey_n})\circ \tilde{i}$ are the bijection of the statement of the lemma.
\end{proof}
Remark from the construction of the reciprocal of $t$ in the latter proof that the injective map $t_{\mathcal{T}}:\mathcal{G}_{\mathcal{T}}\rightarrow \mathcal{G}^m$ such that $t(G)=(G_1,\ldots,G_m)$ satisfies 
$$G=\left(\bigcup_{i=1}^mG_i\right)_{S*S'},$$ 
in the sense of Definition \ref{def:sieving}, where $S,S'\subset \bigcup_{i=1}^m\partial G_i$ are the vertices which are pasted together. 

In Section \ref{subsec:parametrization_honeycombs}, we defined for each $G_i=(V_i,E_i)\in \mathcal{G}_{d_i}$ a map $s:V_i\rightarrow \{-1,1\}$ such that for each edge $\{v,v'\}\in E_i$, $s(v)s(v')=-1$. Denote by $\interior_3(G)$ the set of vertices of degree $3$ of $G$. By the previous definition of $G$ in terms of the graphs $G_i$ and the fact that all vertices of $\interior(G_i)$ have degree $3$, 
$$\interior_3(G)=\bigsqcup_{1\leq i\leq m}\interior(G_i).$$
We can thus paste together the maps $s$ defined on each $G_i$ to get a map 
$$s:\interior_3(G)\setminus \{-1,1\}.$$

\begin{definition}\label{def:map_L_arbitrary_honeycomb}
For $G=(V,E)\in\mathcal{G}_{\mathcal{T}}$, the label map $\mathcal{L}:\honey_{\mathcal{T}}^G\rightarrow \Omega^1(G)$ is the map such that for $(v,v')\in\vec{E}$ with $v\in \interior_3(G)$, 
$$L[h](v,v')=s(v)L(i(e)),$$
where $i(e)$ is the injection from Lemma \ref{lem:product_gluing} and $L$ is the coordinate map defined in Definition \ref{def:triangular_honey}.(2).
\end{definition}
Remark that each edge $e$ of $G$ is then adjacent to at least one trivalent vertex, and if $e=\{v,v'\}$ with $v,v'\in \interior_3(G)$, then necessarily there exists $1\leq i\leq m$ such that $v,v'\in V_i$, and therefore $s(v)s(v')=-1$, so that the latter definition is a well-defined map from $\honey^G_{\mathcal{T}}$ to $\Omega^1(G)$.
...
Let $\gamma^{(1)},\ldots,\gamma^{(p)}\in\mathcal{H}_{reg}$, and recall that $\honey_{\mathcal{T}}^G(\gamma^{(1)},\ldots,\gamma^{(p)})$ denotes the set of honeycomb $h$ on $\mathcal{T}$ such that 
$$(v^j_s)_{\ell_j}=\gamma^{(j)}_s.$$
Let us defined $\phi:V\rightarrow\mathbb{R}$
with 
$$\phi_{\gamma^{(1)},\ldots,\gamma^{(p)}}(v)=\left\lbrace\begin{aligned}
&s(v)&\text{ if }& \deg v=3\\
&1-c(v,v')-c(v,v'')&\text{ if }& \deg v=2, v\sim v', v\sim v'', v'\not=v''\\
&\gamma^{(j)}_s-\delta_{c(v,v')=1}&\text{ if }&v=v^j_s, v\sim v'
\end{aligned}\right..$$
Remark that 
$$\sum_{v\in V}\phi_{\gamma^{(1)},\ldots,\gamma^{(p)}}(v)=\sum_{j=1}^p\sum_{i=1}^n\gamma_s^{(j)}+\sum_{i=1}^m\left(\sum_{v\in \interior(G_i)}s(v)-\sum_{v\in\partial G_i, v\sim v'}c(v,v')\right).$$
\begin{lemma}\label{lem:differential_gluing_honey}
For any $G\in\mathcal{G}_{\mathcal{T}}$, $\mathcal{L}$ is injective and for $\gamma^{(1)},\ldots,\gamma^{(p)}\in\mathcal{H}_{reg}$, 
$$\mathcal{L}(\honey_{\mathcal{T}}^G(\gamma^{(1)},\ldots,\gamma^{(p)}))=\mathcal{F}_{G}(\phi_{\gamma^{(1)},\ldots,\gamma^{(p)}})\cap (K_{G_1}\times \dots\times K_{G_m}),$$
where we write $t(G)=(G_1,\ldots,G_m)$ and $\Omega^1(G)$ is identified with $\prod_{i=1}^m\Omega^1(G_i)$. 
\end{lemma}
We then write 
$$K_{G}=K_{G_1}\times \dots\times K_{G_m}.$$

\begin{proof}
Let $G\in\mathcal{G}_T$. First, by iterating Lemma \ref{lem:product_gluing} on all the glued boundaries, there is a bijection 
$$i:\
\honey_{\mathcal{T}}^G\rightarrow\{(h^1,\ldots,h^m)\in\honey_{T^{1}}^{G_1}\times\ldots\times \honey_{T^m}^{G_m},h^{r_j}_{\vert B_j }=h^{r'_j}_{\vert B_j},1\leq j\leq N(\mathcal{T})\}.$$
Next, since $G=(V,E)$ with $V=\bigcup_{i=1}^mV_i/\langle \partial_{\ell_j}G^{r_j}=\vec{\partial}_{\ell'_j}G^{r'_j},1\leq j\leq N(\mathcal{T})$ and $E$ is the image of $\bigcup_{i=1}^m E_i$ in this quotient. Since any element of $\bigcup_{i=1}^m E_i$ has at most one endpoint in $\bigcup_{j=1}^{N(\mathcal{T})}\vec{\partial}_{\ell_j}G^{r_j}\cup\vec{\partial}_{\ell'_j}G^{r'_j}$, there is thus a canonical identification $\Omega^1(G)=\prod_{j=1}^m\Omega(G^j)$. Moreover, by the definition of $\mathcal{L}:\honey_G^{\mathcal{T}}\to \Omega^1(G)$ from Definition \ref{def:map_L_arbitrary_honeycomb}, for $h\in \honey_G^{\mathcal{T}}$
$$\mathcal{L}(h)=\prod_{i=1}^m\mathcal{L}_{\vert T_i}(h_{\vert T_i})=\left(\prod_{i=1}^m\mathcal{L}_{\vert T_i}\right)(i(h))$$
under the previous identification. The injectivity of $i$ together with the injectivity of each map $\mathcal{L}_{\vert T_i}$ from Proposition \ref{prop:parametrization_triangular_honey_first} yields the injectivity of $\mathcal{L}$.

It remains to describe the image of $\mathcal{L}\left(\honey_{\mathcal{T}}^G(\gamma^{(1)},\ldots,\gamma^{(p)})\right)$. By Lemma \ref{lem:product_gluing} and the previous reasoning, this amounts to describe the image throught $\prod_{i=1}^m\mathcal{L}_{\vert T_i}$ of the set
$$\left\lbrace (h^1,\ldots,h^m)\in\honey_{T^{1}}^{G_1}\times\ldots\times \honey_{T^m}^{G_m},h^{r_j}_{\vert B_j }=h^{r'_j}_{\vert B_j},1\leq j\leq N(\mathcal{T}),h^{s_j}_{\vert L_j}=\gamma^{(j)}, 1\leq j\leq p\}\right\rbrace.$$
Let us first describe how the condition $\partial_{\ell_j}h^{s_j}=\gamma^{(j)}$ for $1\leq j\leq p$ translates through $\mathcal{L}$. Let $v^j_m\in \partial_{\ell_j}G_{s_j}$ and $x\in \partial_{\ell_j}h^{s_j}$ the point such that $i(x)=v^j_m$.  Since $h^{s_j}\in \honey_{G_{s_j}}$, like in Section \ref{subsec:parametrization_honeycombs}, the condition $x_{\ell_j}=\gamma^{(j)}_m$ is equivalent to the condition 
$$\mathcal{L}[h](v^j_m,v)=
\gamma^{(j)}_m-c(v^j_m,v)=0 ,$$
where $v$ is the unique element of $V_{s_{j}}$ such that $v\sim v^j_m$.

Let us now consider the condition $h^{r_j}_{\vert B_j }=h^{r'_j}_{\vert B_j}$. Let $v\in \partial_{\ell^{(1)}_j}G_{r^{(1)}	j}$ and $v'\in \partial_{\ell^{(2)}_j}G_{r^{(2)}	j}$ such that $v\sim v'$ in $G$, and let $x=i_{T^{r^{(1)}_j}}(v), x'=i_{T^{r^{(2)}_j}}(v')$ the corresponding image in $h_{\vert T^{r^{(1)}_j}}$ and $h_{\vert T^{r^{(2)}_j}}$. Using that the edges of both triangles are identified in the order-reversing way, the condition $h^{r_j}_{\vert B_j }=h^{r'_j}_{\vert B_j}$ implies that 
$$x_{\ell^{(1)}_j-1}=1-x'_{\ell^{(2)}_j-1}.$$
 Let $w\in V_{r^{(1)}}$ and $w'\in V_{r^{(2)}}$ such that $v\sim_{G_{r^{(1)}}}w$ and $v'\sim_{G_{r^{(2)}}}w'$. Then, following \eqref{eq:honeycomb_divergence_2}, the previous equality condition is equivalent to $\mathcal{L}[h^{r^{(1)}}](v,w)+c(v,w)=1-\mathcal{L}[h^{r^{(1)}}](v,w)-c(v',w')$, which yields by definition of $\mathcal{L}$,
 $$\mathcal{L}[h](\bar{v},w)+\mathcal{L}[h](\bar{v},w')=1-c(v,w)-c(v',w')=1-c(\bar{v},w)-c(\bar{v},w')).$$
\end{proof}
...
Suppose that $\mathcal{T}'$ is obtained by gluing $\mathcal{T}$ and $T^{m+1}$ along the boundaries $L_p$ and $\partial_0 T^{m+1}$. Then, for all $G\in \mathcal{T}'$ and $\gamma^1,\ldots,\gamma^{p+1}\in \mathcal{H}_{reg}$ with $\sum_{i=1}^{p+1}\vert \gamma_i\vert\in \mathbb{N}$,
\begin{align*}
\sum_{G\in\mathcal{G}_{\mathcal{T}}}&Vol(\mathcal{L}\left[\honey_{\mathcal{T}'}^{G}(\gamma^1,\ldots,\gamma^{p+1})\right])\\
=&\int_{\mathcal{H}_{reg}^{\theta} }\sum_{(G_1,G_2)\in\mathcal{G}_{T}\times\mathcal{G}_{\mathcal{T}}}Vol(\mathcal{L}\left[\honey_{\mathcal{T}}^{G_2}(\gamma^1,\ldots,\gamma^{p-1},u))\right]Vol(\mathcal{L}\left[\honey^{G_1}(\tilde{u},\gamma^p,\gamma^{p+1}))\right]d u,
\end{align*}
...
Remark that 
$$\sum_{v\in V}\phi_{\gamma^{(1)},\ldots,\gamma^{(p)}}(v)=\sum_{j=1}^p\sum_{i=1}^n\gamma_s^{(j)}+\sum_{i=1}^m\left(\sum_{v\in \interior(G_i)}s(v)-\sum_{v\in\partial G_i, v\sim v'}c(v,v')\right).$$
Hence, in order for $\phi_{\gamma^{(1)},\ldots,\gamma^{(p)}}$ to yields a non-empty set $\mathcal{F}_{G}(\phi_{\gamma^{(1)},\ldots,\gamma^{(p)}})$, by Proposition \ref{prop:BijectionTrees} it is necessary that $\sum_{i=1}^p\vert\gamma^{(i)}\vert\in \mathbb{N}$. Hence, $\honey_{\mathcal{T}}^{G_2}(\gamma^1,\ldots,\gamma^{p-1},u)$ is non-empty if and only if $u\in\mathcal{H}_{reg}^\theta$ with $\theta-\sum_{i=1}^{p-1}\vert \gamma^i\vert\mod \mathbb{Z}$. 
...
\begin{align}
\int_{\mathcal{H}_{reg}^\theta }&Vol(\mathcal{L}\left[\honey_{\mathcal{T}}^{G_2}(\gamma^1,\ldots,\gamma^{p-1},u))\right]Vol(\mathcal{L}\left[\honey^{G_1}(\tilde{u},\gamma^p,\gamma^{p+1}))\right]d u\nonumber\\
=&Vol(K_{G_1}\times K_{G_2}\cap \mathcal{F}_{G}(\phi_{\gamma^{(1)},\ldots,\gamma^{(p+1)}}))\nonumber\\
=&Vol(\{\mathcal{L}[h_1],\mathcal{L}[h_2]), (h_1,h_2)\in\honey_{T}^{G_1}\times \honey^{G_2}_{\mathcal{T}} ,h^{1}_{\partial_0T}=h^{2}_{\partial_0\mathcal{T}},\nonumber\\
&\hspace{6cm} h^{1}_{\partial_iT}=\gamma^{(i)}, h^{2}_{\partial_{i-2}\mathcal{T}}=\gamma^{(i)},1\leq i\leq p+1).\label{eq:product_formula_volume}
\end{align}
By Lemma \ref{lem:product_gluing},  the latter volume is non-zero if and only if $(G_1,G_2)=t(G)$ for some $G\in \mathcal{G}_{\mathcal{T}'}$. Hence, applying again \eqref{eq:product_formula_volume} yields 
\begin{align*}
\sum_{(G_1,G_2)\in\mathcal{G}_{T}\times\mathcal{G}_{\mathcal{T}}}&\int_{\mathcal{H}_{reg}^\theta }Vol(\mathcal{L}\left[\honey_{\mathcal{T}}^{G_2}(\gamma^1,\ldots,\gamma^{p-1},u))\right]Vol(\mathcal{L}\left[\honey^{G_1}(\tilde{u},\gamma^p,\gamma^{p+1}))\right]d u\\
=&\sum_{G\in\mathcal{G}_{\mathcal{T}'}, t(G)=(G_1,G_2)}\int_{\mathcal{H}_{reg}^\theta }Vol(\mathcal{L}\left[\honey_{\mathcal{T}}^{G_2}(\gamma^1,\ldots,\gamma^{p-1},u))\right]Vol(\mathcal{L}\left[\honey^{G_1}(\tilde{u},\gamma^p,\gamma^{p+1}))\right]d u\\
=&\sum_{G\in\mathcal{G}_{\mathcal{T}'}}Vol(\mathcal{L}\left[\honey_{\mathcal{T}'}^{G}(\gamma^1,\ldots,\gamma^{p+1})\right]),
\end{align*}
...
\end{proof}
A similar reasoning using Proposition \ref{prop:divergence_contraction} yields the following proposition.
\begin{proposition}
...
\begin{align*}
\sum_{G\in\mathcal{G}_{\mathcal{T}}}&Vol(\mathcal{L}\left[\honey_{\mathcal{T}'}^{G}(\gamma^1,\ldots,\gamma^{p-2})\right])=\int_{\mathcal{H}_{reg} }\sum_{G\in\mathcal{G}_{T}}Vol(\mathcal{L}\left[\honey_{\mathcal{T}}^{G}(\gamma^1,\ldots,\gamma^{p-2},u,\tilde{u})\right])d u,
\end{align*}
...
$L_1,\ldots,L_p,L_{p+1}, L_{p+2}$. Let $\mathcal{M}$ be the oriented surface obtained by gluing $L_{p+1}$ and $L_{p+2}$ in an orientation reversing way and suppose that $\mathcal{M}$ is connected. Then, the following formula holds for the corresponding volume of flat $SU(n)$-connection. 
...
\begin{definition}
A disjoint loops configuration $\mathcal{L}=(S,\Gamma_1,\ldots,\Gamma_p)$ is the data of a compact oriented surface together with $p$ disjoints Jordan curves $\Gamma_1,\ldots,\Gamma_p$ on $S$ and for each $\Gamma_i, \,1\leq i\leq p$ an element $\alpha_i\in \mathcal{H}_{reg}$.
...
\begin{proof}
    ...
    \\
    Let $1\leq i\leq r$. Since $G$ is connected, $G_{\vert \interior(G)}$ is connected and thus $G_{\vert (\interior(G)\cup\{w_i\})}$ is connected. 
    \\
