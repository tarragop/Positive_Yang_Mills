...
Let us fix $n \geqslant 3$ in this paper and let us denote by 
$\mathcal{H}=(\mathbb{R}/\mathbb{Z})^n/S_n$ the spaces of 
conjugation classes of $\U(n)$ obtained by identifying a 
conjugation class with the arguments of the corresponding 
eigenvalues in sorted order. An element of $\mathcal{H}$ 
corresponds to an 
ordered $n$ tuples 
$\theta=(\theta_1\geq \theta_2\geq \dots\geq\theta_n)$ with 
$\theta_1\ldots,\theta_n\in[0,1[$.
The corresponding conjugacy class is then $\Ocal(\theta)$ is 
\begin{equation*}
    \mathcal{O}(\theta) \coloneqq \left\lbrace Ue^{2i\pi\theta}U^*, 
    \,U\in \U(n)\right\rbrace\text{, where } e^{2i\pi\theta}=
    \begin{pmatrix} 
        e^{2i\pi \theta_1}&0&\dots&\\0&e^{2i\pi\theta_2}&&\\ 
        \vdots&&\ddots&\\&&&e^{2i\pi\theta_n}
    \end{pmatrix}.
\end{equation*}

\noindent
Let us denote by $\mathcal{H}_{reg}=\{\theta\in \mathcal{H}, 
\theta_1>\theta_2>\ldots>\theta_n\}$ the set of regular 
conjugacy classes of $\U(n)$, namely the ones of maximal 
dimension in $\U(n)$. We then let $\mathcal{H}_{reg}^0$ denote the subset of $\mathcal{H}_{reg}$ corresponding to the regular conjugacy classes of $\SU(n)$. Namely,
$$\mathcal{H}_{reg}^0=\left\{\theta\in \mathcal{H} \; \middle| \; 
...
boundary (possibly empty) and with a locally flat metric $g$. 
Consider for example $S=\mathbb{C}$ or 
$S=\mathbb{C}\setminus\{0\}$. 
Let us denote by $\mathbb{G}$ the set of non-degenerate 
geodesics of $S$. 
For $(x, y) \in S^2$ and $e$ a geodesic from $x$ to $y$, we then set $\partial e=\{x,y\}$ 
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{Yang_Mills_images_code/toric_honey_example_3_4_5_6_7_sparse.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{Yang_Mills_images_code/toric_honey_example_triangle.pdf}
