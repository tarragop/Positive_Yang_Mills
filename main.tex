...
\\
\\
As a consequence of \eqref{eq:nb_vertices_edges_triangle} below, 
all graphs of $\mathcal{G}^{(g,p)}$ have the same number $n_{g,p} = \frac{3Nn(n+1)}{2}$ 
of edges and $n_{g,p}^{(V)} = Nn^2+\frac{(3N - (p+2g))n}{2} $ of vertices. 
...
for the volume form on $n_{g,p}$-dimensional subspaces induced by the canonical Riemannian metric on $\mathbb{R}^{E^{(g,p)}}$. Hence, 
$$\diff \Vol = \sum_{1\leq i_1<\ldots<i_{n_{g,p}} \leqslant n_{g, p}}\diff x_{i_1} 
\wedge\ldots \wedge \diff x_{i_{n_{g,p}}}.$$
In the following statement, we write $\Vol(K)$ for the integration of $ \diff \Vol$ on a 
$n_{g,p}$-dimensional submanifold and we let $\Delta(x)=\prod_{i<j}(x_j-x_i)$ 
...
As it is proven in {\color{blue} XXXRefXXX}, the partition function 
