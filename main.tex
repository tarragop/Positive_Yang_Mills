...
The latter is a random $\U(n)$-valued connection weighted by its curvature, and the critical points then correspond to flat connections, 
...
Note that this formula suggests a simple geometric description of a flat $\U(n)$-connection on a compact Riemann surface, 
...
$\mathcal{H}_{reg}$.
\\
We associate to $(S,\Gamma_1,\ldots,\Gamma_p)$ a labeled finite tree $\mathcal{T}=(V,E)$ such that vertices are labeled by $\mathbb{R}_{\geqslant 0} \times \mathbb{N}$ and edges are labeled by $\mathcal{H}_{reg}$ as follows :
\begin{itemize}
...
if $v_1$ and $v_2$ are boundary components of a loop. Since loops of $\mathcal{L}$ are non-intersecting, each boundary component corresponds to a unique loop $\Gamma_j$ of $\mathcal{L}$, and we label the corresponding edge $e$ by $\alpha_j$.
\end{itemize}

\noindent
We refer to Figure \ref{fig:fig_loops} for an example of such a configuration of loops and its corresponding 
edge-labeled tree.In the following, let us denote by $d_v$ the degree of a vertex $v\in V$. For $T \geqslant 0$ and $x,y \in \SU(n)$, let us denote by $p_T(x,y)$ the projection of the heat kernel 
on $\U(n)$ to $\mathcal{H}$.
...
A \textit{skeleton} is the data of a labeled finite tree $\mathcal{T}=(V,E)$ such vertices are labeled by $\R_{\geqslant 0} \times \mathbb{N} $ 
...
$$Z_{g, p, T}(\alpha_1, \dots, \alpha_p)=\int_{\mathcal{H}} Z_{g, p, T-T'}(\alpha_1, \dots, \alpha_{p-1}, u) \, 
        Z_{0, 2, T'}(u^{-1}, \alpha_p) \prod_{\ell=1}^p p_{t_\ell}(\alpha_\ell) \Delta(u)^2\diff u \ .$$
Let us shrink the domain between the curve $\gamma$ and the curves $\gamma_1,\ldots,\gamma_p$ so that $T'$ converges to $T$ i.e $T-T'$ converges to zero. 
In this case, the partition function $Z_{g, p, T-T'}(\alpha_1, \dots, \alpha_{p-1}, u)$ converges to $Z_{g,p,0}(\alpha_1, \dots, \alpha_{p-1}, u)$, 
the volume of flat connections with boundary holonomies $\alpha_1,\ldots,\alpha_p$, see \cite{sengupta2003volume}. 
\end{proof}
